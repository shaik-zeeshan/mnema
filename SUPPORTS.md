...
| Render-idle gate (screens-asleep repaint suppression) | [x] | [ ] | [ ] | WebKit strands a non-purgeable IOSurface for every layer repaint that happens while the window cannot reach a lit display, so the frontend pauses its periodic DOM updates between two macOS-only `NSWorkspace` notifications bridged as Tauri events — `screens_did_sleep` / `screens_did_wake` (`native_capture.rs`, observers registered in `start_system_wake_notifier`). This is **not** plain occlusion: `document.visibilityState` stays `visible` through a screen sleep, so the visibility path alone does not cover it. The gate **fails open** — on a build where these notifications never fire, `screensAsleep` stays `false` and `renderIdle()` degrades to `document.visibilityState`, so the UI never wedges paused. `system_did_wake` is kept as a redundant un-gating signal. WebView2/WebKitGTK repaint-surface behavior unassessed; no equivalent notification wired. |
| Offscreen-window clamp on display reconfiguration | [x] | [ ] | [ ] | The Core Graphics display-reconfiguration callback now emits `display_configuration_changed` at the **end** of every reconfiguration (attach, detach, mode change — not just a display coming online), and the main window debounces it to clamp itself back onto a live monitor when a display removal parked it offscreen (`render-idle-clamp.ts`; clamping is opt-in per window — `+layout.svelte` enables it for every window except the dedicated surfaces and Quick Recall, which are positioned deliberately). The pre-existing dark/deep-idle capture-recovery call on that same callback is unchanged — still gated on the display-online flags. Windows/Linux have no equivalent listener wired. |
...
| Ask AI (in-process Reasoning Engine) | [x] | [~] | [~] | Quick Recall + Insights Chat run in-process on the shared Reasoning Engine (`crates/ai-runtime` via `rig-core`) — no installed PI/Node runtime, no shim, no `node`/`pi`-on-PATH resolution. Brokered `search`/`timeline`/`speakers`/`show_text`/`recall_context` tools (plus presentation-only `reference_captures`) are injected from the Tauri layer under the All-Retained Ask AI broker scope. Cross-platform Rust like the engine itself; a cloud engine needs the bring-your-own-key in the Encrypted Capture Index key store (macOS Keychain only today, see that row), a local Ollama/Llamafile engine needs no key and is platform-agnostic. Windows/Linux are blocked only on the platform key store for cloud keys. |
...
| MCP http connector OAuth (browser authorize) | [x] | [ ] | [ ] | OAuth is an **auth mode** on the `Http` transport ([ADR 0051](docs/adr/0051-mcp-oauth-is-an-auth-mode-on-the-http-transport-dcr-only-over-the-deep-link.md)), built on `rmcp`'s `auth` feature (PKCE + RFC 7591 Dynamic Client Registration + `.well-known` discovery + silent refresh; `AuthClient` over reqwest 0.13). Connect runs a foreground browser round-trip whose redirect is captured through Mnema's existing **`mnema://` deep link** (`mnema-dev://` in dev) — the `on_open_url` handler routes `mnema://oauth/callback?code&state` by path, keyed to a CSRF-`state` pending map in `McpManager`; **no loopback listener** (avoids the recurring macOS firewall prompt). The **OAuth Token Set** (access/refresh/expiry/DCR `client_id`) serializes into the **same single vault slot** the static bearer secret uses (secret vault account `mcp-connectors/<instance id>`, unlocked by the one `day.mnema.vault` keychain master item — macOS Keychain only, see that row), so there is no new keychain service, table, or migration. Disconnect/delete makes a best-effort RFC 7009 revocation then always drops the local token. The `mnema://` scheme registration, keychain, and opener are all macOS on this branch; Windows/Linux need their scheme-registration and keychain siblings (blocked on the platform key store, same as the cloud-key rows). |
| ChatGPT subscription provider (device-code login) | [x] | [~] | [~] | The one cloud AI provider whose credential is not a pasted API key: an OAuth **device-code** login against `auth.openai.com`, billed against the user's own ChatGPT Plus/Pro plan through the Codex backend ([ADR 0058](docs/adr/0058-chatgpt-subscription-is-a-provider-kind-with-a-vault-held-oauth-token-set.md)). Unlike the MCP http connector OAuth row above, the device-code flow needs **no deep-link scheme registration and no loopback listener** — the user types a code into a browser page the app opens with `@tauri-apps/plugin-opener`, so the flow is portable by construction. The **OAuth token set** (access + refresh + expiry) serializes into the **same single vault slot** a pasted API key would occupy, unlocked by the one `day.mnema.vault` keychain master item — macOS Keychain only today, so Windows/Linux are blocked on exactly the same platform key store as every other cloud-credential row, and nothing else. |
| User Context / Reasoning Engine | [x] | [~] | [~] | Reasoning Engine + User Context derivation are cross-platform Rust via `rig-core` with no native capture dependency. A cloud engine works on any platform with a credential — either a bring-your-own-key (Anthropic/OpenAI/OpenAI-compatible) or a ChatGPT subscription sign-in (see the row above) — but that credential is stored in the app's encrypted vault, unlocked by a **macOS Keychain only** master item (see the key-store row). A local engine needs a running Ollama/Llamafile endpoint and no key, so it is platform-agnostic. Windows/Linux are blocked only on the platform key store for cloud keys; local-engine derivation should already work cross-platform. |
...
| Broker Authorization Channel | [x] | [ ] | [~] | The per-user Unix socket (`#[cfg(unix)]`) that a local tool asks for CLI Access over. Approval is now a **standing per-tool permission** ([ADR 0059](docs/adr/0059-cli-access-is-a-standing-per-tool-permission-with-idle-expiry.md)): one row per client identity, no calendar expiry, pruned 30 days after last use, widened in place, and revoked as a visible **blocked** state answered without opening a window. Every request opens the `cli-access-request` window — the native message-box quick-approval is deleted — and the CLI's TTY gate is gone, so a piped agent reaches the window. Unix socket implementation works for the macOS/Linux shape; **Windows has no approval path at all** — `send_authorization_request` is a `#[cfg(not(unix))]` stub returning `app_unavailable`, so first-time CLI Access cannot be granted there until a named pipe/TCP sibling exists. |
...
- [x] Screens-asleep/wake notification bridge: `NSWorkspaceScreensDidSleep` / `NSWorkspaceScreensDidWake` observers (registered alongside the sleep/wake pair in `start_system_wake_notifier`) re-emitted as the `screens_did_sleep` / `screens_did_wake` Tauri events. These drive the frontend render-idle gate, not capture — no capture family suspends on them (a screens sleep is already handled as display-unavailable liveness for screen only).
- [x] End-of-reconfiguration display event: the Core Graphics reconfiguration callback emits `display_configuration_changed` for every non-`kCGDisplayBeginConfigurationFlag` callback, including display **removal**, so the frontend can clamp an offscreen-parked window back on-screen. The capture-recovery half of that callback stays gated on the display-online flags, unchanged.
...
- [x] Live system-audio level probe (`start_system_audio_level_probe` / `get_system_audio_probe_level`, `crates/capture-system-audio/src/probe.rs`): a short-lived Core Audio process tap reporting the peak level heard since the last poll, so onboarding's Permissions screen can show sound *arriving right now*. System audio has no readable grant (ADR 0052), so building the tap IS the "Screen & System Audio Recording" prompt — `Ok` means the tap runs, never that it was granted (a denied tap delivers zeros). Deliberately separate from `crate::activity`, so a probe run from a settings screen cannot teach the recording session's inactivity or denial heuristic anything. Self-stopping ~3 s after the last poll (polling is the keepalive), so no start/stop pair can leak a tap. Non-macOS builds stub both commands.
- [x] Voice enrollment (`enroll_account_owner_voice` / `get_account_owner_person_id`, `crates/speaker-analysis/src/enrollment.rs`): a bounded microphone clip is diarized through the shipped `speakrs` provider and either stored as the account owner's voiceprint (`person_profiles.is_account_owner`, at most one row by partial unique index, migration `0053`) or refused with a typed reason (too short / no speech / multiple speakers). Enrolling switches `recognize_saved_people` on; "label my voice automatically" then auto-links only `High` owner suggestions, recorded as `recording_speaker_clusters.person_link_auto` so the user can see and undo it. The clip never enters the capture store — it is judged and destroyed. Tied to macOS by `speakrs` (CoreML + OpenBLAS, Apple Silicon) and AVFoundation decode.
- [x] Screen Recording is onboarding's one hard permission gate: without it there is nothing to record. Enforced in `PermissionsScreen.svelte` against live OS state (not the resolved settings), including the app relaunch a same-process macOS grant needs before ScreenCaptureKit can use it. Every other permission gates nothing, because macOS never re-prompts after a denial.
...
- [x] Broker Authorization Channel over per-user Unix socket, granting a standing per-tool CLI Access permission through the `cli-access-request` window (no native message box, no TTY gate, no calendar expiry — [ADR 0059](docs/adr/0059-cli-access-is-a-standing-per-tool-permission-with-idle-expiry.md)).
- [x] Ask AI (Quick Recall + Insights Chat) brokered tool-calling session: runs in-process on the shared Reasoning Engine (`crates/ai-runtime` via `rig-core`, `run_agent_loop` in `agent_loop.rs`) — NOT the user's installed PI/Node runtime, which is removed (no shim, no `node`/`pi`-on-PATH resolution, no PI auth). It uses the same OS-Keychain provider key as User Context derivation and exposes only the `search`/`timeline`/`speakers`/`show_text`/`recall_context` brokered data tools (plus the presentation-only `reference_captures`), injected from the Tauri layer and enforced through the All-Retained Ask AI broker scope (`BrokeredCaptureAccess::execute_for_ask_ai`). Gating is two-layer: the engine-configured prerequisite plus the independent Ask AI Setting. See [ADR 0033](docs/adr/0033-ask-ai-migrates-onto-shared-reasoning-engine.md).
...
- [ ] Implement Broker Authorization Channel for Windows.
  ...
  - Only the transport is missing. The permission model, the `cli-access-request` window, blocking/enabling in Access Settings, idle expiry, and the CLI's reason-code → exit-code mapping are all platform-neutral and already work; Windows needs the socket sibling and nothing else ([ADR 0059](docs/adr/0059-cli-access-is-a-standing-per-tool-permission-with-idle-expiry.md)).
- [ ] Update `crates/cli` authorization request path for Windows. Today `send_authorization_request` is a `#[cfg(not(unix))]` stub returning `app_unavailable`, so no approval can ever be requested on Windows.
...
- [~] Verify Ask AI (in-process Reasoning Engine) on Windows. There is no PI/Node runtime or shim to spawn — the agent loop runs in-process via `rig-core`. Verify brokered `search`/`timeline`/`speakers`/`show_text`/`recall_context` tool calls round-trip, the All-Retained broker scope/redaction holds, streaming/cancellation/background-completion work, and engine status reports correctly. Cloud-engine use is gated on the Windows Capture Index Key Store (above); a local Ollama/Llamafile engine needs no key.
...
- [~] Broker Authorization Channel can likely reuse Unix socket shape, but needs Linux config-dir/runtime-dir review. Everything above the transport — the standing per-tool permission, the approval window, blocking, idle expiry ([ADR 0059](docs/adr/0059-cli-access-is-a-standing-per-tool-permission-with-idle-expiry.md)) — is inherited unchanged, since the channel compiles under `#[cfg(unix)]`.
...
- [~] Verify Ask AI (in-process Reasoning Engine) on Linux. There is no PI/Node runtime or shim to spawn — the agent loop runs in-process via `rig-core`. Verify brokered `search`/`timeline`/`speakers`/`show_text`/`recall_context` tool calls round-trip under the All-Retained broker scope, streaming/cancellation/background-completion work, and engine status reports correctly across desktop environments. Cloud-engine use is gated on the Linux Capture Index Key Store; a local Ollama/Llamafile engine needs no key.
