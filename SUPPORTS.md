...
| Licensing & Trial | [x] | [ ] | [ ] | Offline one-time License + 30-day Trial → Read-Only Mode, verified on-device against a hardcoded Ed25519 public key ([ADR 0044](docs/adr/0044-monetize-as-one-time-purchase-with-paid-update-window.md)/[0045](docs/adr/0045-licenses-verified-offline-ed25519-polar-merchant-of-record-only.md)). Verification core (`crates/app-infra/src/license_verify.rs`) and the state/gate are cross-platform Rust, but the license-key + trial-record store (`license_token_store.rs`, service `com.shaikzeeshan.mnema.licensing`) is **macOS Keychain only** — the same platform-key-store gap as the rows above. Update-Window enforcement is auto-updater-only (declines builds dated after `update_through`), never a runtime capture lock. |
