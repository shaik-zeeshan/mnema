...
| Speaker analysis | [x] | [x] | [~] | macOS decodes via AVFoundation; Windows decodes a captured `.m4a` through the `media-decode` MF Source Reader seam (ADR 0024), resamples to 16 kHz mono in-crate, and runs the Sherpa ONNX diarization/embedding runtime CPU-only in the isolation subprocess helper. `sherpa-onnx` links via ORT's `shared` feature, so it needs `onnxruntime.dll` + `sherpa-onnx-c-api.dll` at runtime (the build copies them into `target/<profile>/` for dev/test; staging them into the packaged NSIS bundle is still pending — tracked under #82, separate from the statically-linked Parakeet path). On-device diarization of a real multi-speaker recording is deferred to an operator. |
...
- [x] Wire speaker analysis through the `media-decode` seam on Windows. A captured `.m4a` decodes via the MF Source Reader seam, resamples to 16 kHz mono in-crate, and runs the Sherpa ONNX diarization/embedding runtime CPU-only inside the `MNEMA_SPEAKER_ANALYSIS_HELPER` isolation subprocess (the helper resolves `current_exe`, round-trips request/response JSON as raw bytes over stdin/stdout, and enforces a timeout with `kill_on_drop`). `sherpa-onnx` links via ORT's `shared` feature, so it needs `onnxruntime.dll` + `sherpa-onnx-c-api.dll` at runtime — its build copies them into `target/<profile>/` for dev/test, but staging them into the packaged NSIS bundle is still pending (tracked under #82, separate from the statically-linked Parakeet `ort` path; see #80). DirectML/GPU stays deferred. On-device diarization of a real multi-speaker recording is deferred to an operator.
