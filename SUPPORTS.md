...
| Native screen capture | [x] | [~] | [ ] | macOS uses ScreenCaptureKit / AVFoundation fallback. Windows uses WGC for primary-monitor screen capture — the same single primary-display, whole-display scope macOS captures (neither does per-window or multi-display capture). The capture backend itself is at parity; the row stays `[~]` only because transient-liveness / sleep-wake recovery is still maturing. |
