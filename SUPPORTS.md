...
| Native microphone capture | [x] | [ ] | [ ] | macOS uses AVFoundation. `AVCaptureSession.startRunning` blocks and against a wedged input device never returns, so the start is bounded at 20 s (the same cap as every ScreenCaptureKit wait) and fails with `microphone_start_timeout`; the blocked thread is abandoned and stops the session itself if it ever starts late. Any other platform's blocking session start needs the same bound. |
