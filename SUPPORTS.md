...
| Speaker analysis | [x] | [x] | [~] | macOS decodes via AVFoundation; Windows decodes a captured `.m4a` through the `media-decode` MF Source Reader seam (ADR 0024), resamples to 16 kHz mono in-crate, and runs the Sherpa ONNX diarization/embedding runtime CPU-only in the isolation subprocess helper. `sherpa-onnx` links via ORT's `shared` feature, so it needs `onnxruntime.dll` + `sherpa-onnx-c-api.dll` at runtime (the build copies them into `target/<profile>/` for dev/test, and `apps/desktop/src-tauri/build.rs` stages all four DLLs into `resources/windows/`, which `tauri.windows.conf.json` declares as bundle resources installed next to the packaged `mnema.exe`, separate from the statically-linked Parakeet path). On-device diarization of a real multi-speaker recording is deferred to an operator. |
