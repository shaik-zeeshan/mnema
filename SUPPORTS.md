...
| Audio decode to mono PCM (`media-decode` seam) | [x] | [~] | [ ] | macOS keeps its AVFoundation decoders in-crate. Windows decodes `.m4a`/`.mp4` audio to native-rate mono `f32` via the `crates/media-decode` MF Source Reader seam (ADR 0024). Transcription still resamples in-crate; only system-audio speech activity is wired through the seam so far. |
...
| Live app privacy exclusion | [x] | [ ] | [ ] | macOS uses ScreenCaptureKit app exclusion filters. Windows v1 ships none ([ADR 0025](docs/adr/0025-windows-v1-ships-without-app-privacy-exclusion.md)); a future Windows mechanism is capture-suspension-shaped, not a WGC pixel filter, and needs its own ADR. Linux semantics still need design. |
| Active app/window metadata | [x] | [ ] | [ ] | macOS uses NSWorkspace/CoreGraphics. Windows v1 design decided ([ADR 0043](docs/adr/0043-windows-v1-active-window-metadata-and-app-identity.md)): foreground-window canonical-exe-path identity (opaque, stored in the existing `app_bundle_id` field, no rename) + `SetWinEventHook(EVENT_SYSTEM_FOREGROUND)` refresh; AUMID/PFN + UWP correctness deferred. Implementation pending. |
| Browser URL metadata | [x] | [ ] | [ ] | macOS reads the active-tab URL through a per-browser **Browser URL Strategy**: AppleScript for supported Chromium and WebKit browsers (no extra permission), and the macOS Accessibility API for Firefox-family (Gecko) browsers — Firefox and Zen — reading `AXURL` off the focused web area. The Accessibility path is opt-in and Gecko-only: it requires the macOS Accessibility permission; if the permission is not granted, Gecko browsers yield no URL. See [ADR 0039](docs/adr/0039-gecko-browser-active-tab-url-via-accessibility-api.md). Windows: **deferred for v1** and reported unsupported ([ADR 0043](docs/adr/0043-windows-v1-active-window-metadata-and-app-identity.md)); revisit via a UI Automation prototype in its own ADR, adding a mirrored permission-grant UX only if the chosen source needs an OS permission. |
...
| Status bar / tray | [x] | [~] | [~] | Tauri tray exists cross-platform; current UX includes macOS-only Exclude Current App behavior. **Decided:** Windows v1 hides the "Exclude Current App" tray item entirely (it currently shows a dead "only on macOS" dialog), since Windows has no app-exclusion feature ([ADR 0025](docs/adr/0025-windows-v1-ships-without-app-privacy-exclusion.md) / [ADR 0043](docs/adr/0043-windows-v1-active-window-metadata-and-app-identity.md)); implementation pending. |
...
- [ ] Implement active app/window metadata. **Decided (ADR 0043):** foreground `HWND`→PID→`QueryFullProcessImageNameW` canonical exe path (opaque identity), `GetWindowTextW` title (Metadata-Settings-gated), display name from version-info `FileDescription`→file stem; refreshed via a `SetWinEventHook(EVENT_SYSTEM_FOREGROUND)` listener (light callback, signal-only) plus the existing cross-platform 1s segment poll as fallback; macOS's token/generation coalescer is not ported (it guards the live filter Windows lacks). Implementation pending.
...
- [x] Design live app privacy exclusion semantics. **Decided:** v1 ships none (ADR 0025); a future Windows mechanism is capture-suspension-shaped, not a WGC pixel filter, and requires its own ADR before being offered in any UI.
  - If per-app exclusion is not possible, expose clear unsupported/degraded UX and rely on pause/delete-recent/app disclosure.
- [ ] Implement “Exclude Current App” using Windows active-window identity, or hide/disable it. **Decided (ADR 0025/0043):** hide the tray item entirely on Windows — there is no app-exclusion feature to add to, and a dead "only on macOS" item is worse than absence; not repurposed to the separate "hide Mnema's own windows" concept. Implementation = platform-gate the item in `status_bar.rs`. Pending.
- [ ] Add Windows sensitive app recommendation catalog.
- [ ] Add Windows known-browser catalog and browser capture disclosure.
- [x] Decide whether browser URL metadata is supported on Windows; do not add browser extension plumbing without an ADR. If the chosen source needs an OS-level permission (the way Gecko URLs need macOS Accessibility), add an equivalent permission-grant/recheck UX in Settings and onboarding mirroring the macOS Gecko Accessibility flow. **Decided (ADR 0043):** deferred for v1 — Windows reports browser URL unsupported; ship active app/window metadata first. Revisit as its own ADR (UI Automation prototype), adding the mirrored permission UX only if the chosen source needs an OS permission.

### Storage, access, and release

- [x] Implement Windows Capture Index Key Store using Credential Manager, DPAPI, or another platform-owned secret store.
- [ ] Port low-disk capture safety to Windows. The `LowDisk` Capture Suspension design is platform-neutral (and already implemented on macOS): probe free space at each new-segment boundary (preflight + rotation), refuse to start / suspend all sources / hysteresis auto-resume around a `1 GiB` reserve floor plus one-segment estimate, and stop gracefully below the floor. The same discard-the-partial behavior also covers the Windows Media-Foundation file-lock case, since partials are discarded rather than left locked. See [ADR 0040](docs/adr/0040-low-disk-safety-is-a-transient-liveness-capture-suspension-kind.md).
...
- [~] Add Windows install/update signing process. Updater minisign signing works via the shared `TAURI_SIGNING_PRIVATE_KEY` (same key as macOS). Authenticode code signing is **not** provisioned yet: the `release.yml` signing step is a secret-gated no-op stub when the cert secret is absent, so the NSIS installer and auto-updates ship **unsigned** and Windows SmartScreen shows an "unknown publisher" warning on both install and update — acceptable for preview/smoke only. Azure Trusted Signing is intended as the gate before stable-channel non-technical users. See [docs/release-process.md](docs/release-process.md).
