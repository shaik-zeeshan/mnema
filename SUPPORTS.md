...
| Speaker analysis | [x] | [ ] | [ ] | On-device diarization runs through the `speakrs` provider (pure-Rust pyannote-community-1 segmentation + WeSpeaker embedding + VBx clustering on CoreML), which links system OpenBLAS. CoreML ties it to Apple Silicon macOS; audio decode is AVFoundation-only too. There is no Windows/Linux on-device diarizer: the earlier Windows Sherpa-ONNX path was removed when speakrs became the sole provider, and speakrs cannot run off Apple Silicon. The `media-decode` seam can still decode audio on Windows for transcription/speech-activity, but the subprocess dispatcher reports diarization `ProviderUnavailable` off macOS. A non-CoreML diarization provider is the deferred gap for Windows/Linux. |
...
- [x] Wire speaker analysis through the `media-decode` seam on Windows. A captured `.m4a` decodes via the MF Source Reader seam, resamples to 16 kHz mono in-crate, and runs the Sherpa ONNX diarization/embedding runtime CPU-only inside the `MNEMA_SPEAKER_ANALYSIS_HELPER` isolation subprocess (the helper resolves `current_exe`, round-trips request/response JSON as raw bytes over stdin/stdout, and enforces a timeout with `kill_on_drop`). `sherpa-onnx` links via ORT's `shared` feature, so it needs `onnxruntime.dll` + `sherpa-onnx-c-api.dll` at runtime — its build copies them into `target/<profile>/` for dev/test, and `apps/desktop/src-tauri/build.rs` stages all four DLLs into `resources/windows/`, which `tauri.windows.conf.json` declares as bundle resources installed next to the packaged `mnema.exe` (separate from the statically-linked Parakeet `ort` path; see #80). DirectML/GPU stays deferred. On-device diarization of a real multi-speaker recording is deferred to an operator.
...
| `crates/speaker-analysis/src/macos_audio_decode.rs` | AVFoundation audio decode for diarization; the `speakrs` provider itself is CoreML + OpenBLAS (Apple Silicon only) | macOS keeps AVFoundation decode in-crate; Windows/Linux need a non-CoreML diarization provider, since `speakrs` cannot run off Apple Silicon (the earlier Windows Sherpa-ONNX path was removed when speakrs became the sole provider) |
