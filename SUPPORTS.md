...
| Timed off-the-record (auto-resuming capture pause) | [x] | [ ] | [ ] | "Off the record" is the user pause of all capture families; the timed variant carries a wall-clock deadline (persisted in the `app_settings` kv row `capture.off_record_deadline_unix_ms`) after which capture auto-resumes. Resume survives system sleep by design: both the segment-loop tick (live paused session) and the startup watcher (deadline re-armed after a relaunch; skips auto-start so nothing records inside the window) compare the wall-clock deadline on every turn — never a monotonic timer. The state/persistence halves are cross-platform Rust, but the resume drivers run inside the macOS-gated capture runtime and are exercised on macOS only; tray copy ("Go Off the Record" / "Back On the Record", countdown header) rides the cross-platform tray. |
