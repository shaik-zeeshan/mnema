...
- **ORT DLL placement (resolved for Parakeet, #80):** the Parakeet `ort` dependency uses its default features (`download-binaries`, no `load-dynamic`), so `ort-sys` emits `cargo:rustc-link-lib=static=onnxruntime` and links ONNX Runtime *statically* into `mnema.exe` — the packaged Windows app needs no separate `onnxruntime.dll` for Parakeet. CI exercises this link via `cargo build -p mnema --lib` on `windows-latest` (the `windows-check` job), so it is verified, not asserted. The remaining open piece is the **Sherpa speaker path**, which deliberately uses `sherpa-onnx`'s `shared` feature (→ `dylib=onnxruntime` + `sherpa-onnx-c-api.dll`); its build copies those DLLs into `target/<profile>/` for dev/test but they are **not yet staged into the Tauri bundle** (`tauri.conf.json` declares no `resources` for them), so a packaged NSIS install would not find them. Tracked against the speaker-analysis item (#82); options are bundling the DLLs as Tauri `resources` or switching Sherpa to a static archive as macOS does.
