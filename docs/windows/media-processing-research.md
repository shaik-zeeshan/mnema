...
- **ORT DLL placement (resolved, #137):** on Windows the whole binary now builds with `ort/load-dynamic` (added under `[target.'cfg(target_os = "windows")'.dependencies]` in `crates/audio-transcription/Cargo.toml`, so feature unification keeps macOS static `download-binaries`). ONNX Runtime is therefore loaded at runtime from a bundled `onnxruntime.dll` (ONNX Runtime 1.24.4, MIT, version-locked to `ort = =2.0.0-rc.12`) shipped flat next to the exe — staged by `scripts/prepare-ort-dylibs.mjs`, declared as a Windows Tauri resource in `tauri.windows.conf.json`, and resolved via `ORT_DYLIB_PATH` (`apps/desktop/src-tauri/src/ort_dylib.rs`) in both the main app (in-process Parakeet) and the re-invoked speaker-analysis helper. **Parakeet AND speakrs share that one dynamic ORT node** — there is no longer any statically-linked ORT on Windows, and there is no separate Sherpa path (speakrs is the sole diarizer, `sherpa-onnx`/`sherpa-onnx-c-api.dll` are gone). The opt-in CUDA Execution Backend extends this same base install with `onnxruntime_providers_shared.dll` + `onnxruntime_providers_cuda.dll` (from a CUDA-enabled ORT 1.24.x build, sourced via `MNEMA_ORT_REDIST_ZIP`) plus the on-demand NVIDIA CUDA 12 + cuDNN 9 redist pack loaded off the search path. See ADR 0005 (`crates/speaker-analysis/docs/adr/0005-windows-cuda-backend-in-app-provisioning-and-dynamic-ort.md`) and the operator matrix `crates/speaker-analysis/docs/windows-cuda-verification.md`.
