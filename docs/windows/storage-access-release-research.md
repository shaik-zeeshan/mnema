...
| CLI sidecar target suffix script supports `.exe` | `scripts/prepare-mnema-cli-sidecar.ts` | Bun-based cross-platform script invoked via `bun run`; runs on `windows-latest` without Git Bash. |
...
- `scripts/prepare-mnema-cli-sidecar.ts` already appends `.exe` for Windows target triples.
