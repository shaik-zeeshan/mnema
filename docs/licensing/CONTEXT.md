...
- **The Device Cap can be overshot across datacenters.** Slot claims go through Cloudflare KV (~60s cross-datacenter propagation); two brand-new machines activating through different datacenters within that window can both succeed, putting a 4th machine on the 3-cap. Within one datacenter the claim-then-re-verify path is race-safe (pinned by `test/cap-race.test.ts`). Accepted because exploiting it requires orchestrating simultaneous cross-region activations per extra device — not casual sharing, which is the cap's actual target — and `lifetime_machine_count` still increments, so the leak telemetry that feeds the **Revocation List** sees the abuse regardless. Revisit trigger: lifetime counts showing real over-cap abuse in the wild → move the slot set into a Durable Object keyed by license id.
