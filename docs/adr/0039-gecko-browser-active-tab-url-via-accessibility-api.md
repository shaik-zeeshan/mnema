...
The reader is built on hand-rolled `extern "C"` FFI against the `ApplicationServices` framework (no new crate dependency). It bounds a slow/hung browser with a **wall-clock budget per read attempt** (≈400ms): `AXUIElementSetMessagingTimeout` (0.5s) only bounds a single AX message, but one read issues ~50 of them (`AXFocusedUIElement` plus up to 16 parent hops, each reading role/URL/parent), so without a wall-clock cap a slow-but-responding browser could stall one attempt for ~25s. The parent-hop climb stops early once the per-attempt budget is spent, so the worst case for one attempt is the budget plus at most one in-flight message timeout. It also polls the first read (≤500ms, 50ms steps) to wake a dormant a11y engine — measured cold→live latency is ~100–150ms. This wall-clock bound is what makes the synchronous inactivity-resume read safe, since that read runs while the capture-state mutex is held and must not block stop/refresh/capture for seconds.
...
- **Graceful degradation.** With the permission not granted (or the read failing), Gecko browsers degrade to today's blank-URL behavior. The per-attempt wall-clock budget (≈400ms) — not the per-message 0.5s timeout alone — plus the ≤500ms first-read poll bound the cost of a slow, hung, or dormant browser so a stuck read cannot stall capture (including the synchronous inactivity-resume read that holds the capture-state mutex).
