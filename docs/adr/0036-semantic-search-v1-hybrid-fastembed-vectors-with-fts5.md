...
> **Partially superseded by [ADR 0037](0037-semantic-search-embeddings-on-candle-with-pluggable-backend.md).** The **"Runtime"** and **"Model selection"** sections below are superseded: embeddings now run on **candle** (on-device GPU/CPU) behind a pluggable **Semantic Search Backend** rather than `fastembed`/ONNX, and the model catalog is hand-maintained rather than synthesized from fastembed's `ModelInfo`. Everything else in this ADR — the hybrid FTS5 ⊕ `vec0` RRF substrate, filter-then-rank, the atomic model switch, the encrypted-at-rest + egress-redaction privacy model, and model-gated default-on rollout — still holds.
>
> **Also superseded (2026-08-01, issue #190/#193): the "one dimension authority" of §2 and §4 below.** The live `vec0` column width is no longer the discriminator between embedding spaces on the **write** path or in **startup reconciliation** — the catalog now ships models that deliberately share a width (`gte-modernbert-base` and `granite-embedding-english-r2` at 768 = nomic; `granite-embedding-small-english-r2` at 384 = e5), which a width compare cannot tell apart. Both paths now gate on an **index epoch** stamped in `app_settings` (`model_id@EMBED_INDEX_RECIPE`), written in the same transaction as the table's DROP+CREATE. See the amendments to [ADR 0037](0037-semantic-search-embeddings-on-candle-with-pluggable-backend.md). The **query** path in §3 is unchanged: it still degrades to keyword-only on a live-width mismatch.
