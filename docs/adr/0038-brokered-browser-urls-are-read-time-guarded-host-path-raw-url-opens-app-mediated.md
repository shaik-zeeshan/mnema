---
status: accepted, amended 2026-07-26 (the guarded URL is also projected into the search index — see Amendment)
---

# Brokered browser URLs are a read-time guarded host+path; the raw URL opens app-mediated

Accepted. Amended 2026-07-26: a **guarded** URL is now also projected into the search index to back a URL *filter*, partially reversing this ADR's rejection of an index column (see Amendment).
...
1. **Broker (cloud-facing): a guarded host+path text — the Broker URL Context.**
   ...
   - **Read-time, from the representative frame, gated on data presence.** Computed at broker-return by joining the result's representative `Captured Frame` → metadata snapshot. (**Amended 2026-07-26:** the read-time guard is unchanged, but the guarded text is *also* written to a `search_documents.url` column — migration `0050` plus a one-time startup backfill — so a URL *filter* has a SQL-visible predicate. "No new index column, no backfill, covers history for free" no longer holds; see Amendment.) Returned as `Search Context` on **both** search results and timeline intervals (an interval is page-granular via frame equivalence, so its representative frame's URL is accurate). Gated on the frame *carrying* a URL, not on the current `BrowserUrlMode`.
...
- **Project a redacted `browser_url` column into the search index.** Rejected: persists an unguarded/pre-policy URL into the index (itself broker-visible text), needs a migration + backfill, and reopens persist-time redaction. The read-time join keeps the guard at the boundary and covers all history. **Partially reversed 2026-07-26** once a URL *filter* needed a SQL-visible predicate — but only for the **guarded** text, which is why the original objection survives intact (see Amendment).
...
- The guard runs **read-time**, diverging from secret redaction's prospective persist-time gate ([ADR 0015](0015-secret-redaction-v2-is-prospective.md)/[0016](0016-secret-redaction-v2-deterministic-gate.md)) — by design, so it covers all historical URLs with no migration. (Since 2026-07-26 it *also* runs write-time to fill the filter column; the read-time path is unchanged. See Amendment.)
...
## Amendment (2026-07-26): the guarded URL is also projected, to make it filterable

This ADR rejected "project a redacted `browser_url` column into the search index" — but that rejection was aimed at a column holding the **raw or capture-time-sanitized** URL. Output context was the only consumer then, and a read-time join served it for free.

A URL *filter* (`mnema search --url` / `--url-regex`, same on `timeline`) has a different requirement: a predicate has to run inside the SQL that selects rows, and `browser_url` lives only in `frame_metadata_snapshots.snapshot_json`. Filtering read-time would mean JSON-parsing every candidate row.

Changes:

- **`search_documents.url` holds the output of `guard_url`, never the raw URL** (migration `0050`). The rejected option's core objection — "persists an unguarded/pre-policy URL into the index" — does not apply: what is persisted is exactly the text the broker already emits, so the index gains no string the boundary would refuse to hand out.
- **This is also what makes the filter safe.** Matching the raw URL would let a grant-holding client filter on data the guard exists to withhold — probing for a query-string value or a path secret by asking whether any row matches it, without ever being shown one. Filtering the guarded form makes the filter's reach identical to the output's reach, so it is not an oracle.
- **The guard now runs at both times.** Read-time guarding of the representative frame's URL for `Search Context` is unchanged. Write-time guarding at projection is additive, and moves that cost onto the capture-rate path (measured 2.3 µs/call release; frames without a URL short-circuit).
- **A backfill covers history**, mirroring the existing app-identity backfills: a read-pool scan over frame-anchored rows, guarded in Rust, applied in batched write transactions. So "covers history for free" is no longer free, but it is a one-time startup cost, not a per-query one.
- **Filters inherit the guard's blind spots, visibly.** Query strings and fragments are never indexed, so a filter containing `?` or `#` matches nothing, and a redacted path segment is unmatchable. Documented in the `mnema-data` skill so agents filter on host and readable path segments.

Unchanged: the broker never emits a raw URL, never opens one, and `OpenCapturedUrl` is still rejected for every caller. `--url` is a case-insensitive substring; `--url-regex` is a case-sensitive `regex`-crate pattern (linear-time, so a client-supplied pattern cannot wedge the reader) validated in Rust before it reaches SQLite.
