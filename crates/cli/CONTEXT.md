...
- A speaker `handle` is the only way to address a person through the CLI; names are never a filter, because two profiles can share a display name and most voices have no name at all. A `person` handle is stable across recordings, channels, and renames. A `voice` handle is one voice inside ONE recording — it fragments across the same human and dies on re-diarization, so nothing may persist one: a saved view, scheduled digest, or trigger holding a `voice` handle silently returns nothing.
