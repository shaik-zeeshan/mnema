...
- Mnema CLI search output returns opaque result `id`, kind, redacted snippet, startedAt, endedAt, the effective (server-clamped) limit, and `nextCursor` — present only when more matches remain. `--cursor` walks the pages: the cursor pins the walk to the search-document snapshot it started on and carries per-anchor offsets, so a walk never re-reads or skips rows as new captures land. (V1 shipped `nextCursor` as an always-`null` field; it is now real.)
