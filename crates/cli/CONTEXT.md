...
- Mnema CLI data commands expose no scope option; the needed scope is derived from the command's own `--from` (`needed_scope_for`) and sent as the channel `minimum`, so the approval window can never offer a choice that would leave the caller short.
...
- **Broker Authorization Channel** requests include schema version, request ID, **Broker Client Identity** label and provenance, command type, minimum and preferred scope, interactivity, and creation timestamp. There are no duration fields.
