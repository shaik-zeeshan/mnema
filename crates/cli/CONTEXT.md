...
- `mnema search --speaker <handle>` and `mnema timeline --speaker <handle>` narrow to audio the person or voice was heard in, and matching results carry that speaker's words inline as `turns` — the 2-call workflow (`speakers`, then one filtered query), never one `show-text` per result. The filter matches voices the user assigned AND voices recognition guessed at with no assignment, so results can include unconfirmed matches.
