...
- **Broker Authorization Channel** endpoint discovery is a deterministic per-user endpoint derived from the configured desktop app identifier, and CLI/server code should not duplicate that identifier as an unrelated hardcoded constant. macOS uses a per-user runtime or temporary-directory Unix domain socket path; Windows uses a named pipe `\\.\pipe\{identifier}-{current-user-sid}-cli-access`, with the SID computed from each process's own token so no durable discovery file exists ([ADR 0045](../../docs/adr/0045-windows-broker-authorization-channel-via-named-pipe.md)).
...
- Mnema CLI treats an existing but unconnectable **Broker Authorization Channel** socket path as a stale endpoint, but stale socket unlinking is owned by the desktop app startup/bind path rather than the CLI. This is macOS-specific: a Windows named pipe is a kernel object with no stale-file artifact, so there is nothing to unlink — pipe absence is simply the app-unavailable case, and `first_pipe_instance` owns endpoint ownership ([ADR 0045](../../docs/adr/0045-windows-broker-authorization-channel-via-named-pipe.md)).
...
- **Broker Authorization Channel** keeps the domain boundary abstract across local IPC implementations: macOS uses a Unix domain socket and Windows uses a named pipe ([ADR 0045](../../docs/adr/0045-windows-broker-authorization-channel-via-named-pipe.md)), sharing one newline-framed JSON protocol and one connection handler generic over the stream. Linux would reuse the Unix-socket path. A future Linux or other local IPC implementation stays within the same boundary.
