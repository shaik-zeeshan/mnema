...
- **Brokered Capture Access** audit history stores one non-content event per brokered command — client identity and provenance, command type, timestamp, result count, scope class, issuing grant id, and an `outcome` of `success`, `denied`, or `scope_rejected` — never raw query text, returned snippets, OCR text, transcripts, browser URLs, or media paths. Refused commands are recorded too: a permission log that only records successes cannot answer "did anything try and get turned away".
- The audit file is written only by the external-tool path (`execute_for_identity`). Ask AI does not write to it: its agent loop emitted one event per tool call and evicted real CLI history from the 500-slot FIFO, and it already shows its sources per answer. Blocking and enabling a client are permission-file edits and are not audited.
...
- **Brokered Capture Access** grant and audit files are schema-versioned. There is no back-compatibility with the pre-ADR-0059 shape: per that ADR, "the new shape is written and any old file is ignored" — an old row carries no `lastUsedAtUnixMs`, so it reads as long idle-expired and is pruned, and the next call re-prompts. An EMPTY (zero-length) grant file is the one recovery case, not a legacy one: it is the crash artifact of the temp+rename write, holds no permission and no block, and reads as no rows so it cannot wedge the approval that is the only way out. Non-empty garbage still fails closed — it may be a mangled block list.
- **Brokered Capture Access** writes its three files — the permission file, the audit file, and the opaque-id MAC secret — owner-read/write only (`0600` on unix). The app-support directory is already `0700`, so this is the second fence; the mode rides the `open` of the TEMP file because a `rename` keeps the temp file's mode, and a chmod after the rename would publish the bytes for the window in between. The opaque-id secret is a key, and the permission file is now rewritten on every hourly stamp and every prune.
- An opaque result id's tag is a secret-prefix MAC — `SHA-256(secret ‖ ':' ‖ payload)` truncated to 128 bits — not HMAC. Sound only because of three properties, all of which must survive any refactor of the id shape: the truncation withholds half the digest state (no length extension), the kind and row id are the payload HEAD (an append cannot retarget them), and the grant id runs to the END with the FIRST `:g` winning the parse (an appended `:g<other grant>` lands inside the grant id and matches no row). A payload with attacker-chosen or trailing-significant fields needs real HMAC first.
- Loading a **Brokered Capture Access** grant file display-normalizes labels but must NOT rewrite `normalized_label` into anything other than what `normalize_client_label` produces for the row's own label: that key is the one-row-per-identity invariant, and a load-time rewrite disagreeing with the upsert's lookup silently re-opens the duplicate-row bug. (The old `Local agent` → `mnema CLI` rewrite did exactly that; it was a pre-rename shim and there are no installed users, so it is gone.)
