...
**Open Captured URL**:
The app-mediated action that opens a **Captured Frame**'s original browser URL in the user's default browser. Like the CLI `open` action, it is keyed off the opaque result id and resolved locally by the app — the raw URL is never returned as broker text and never reaches a cloud model, materializing only when the user clicks. Surfaced wherever a captured web moment is shown (first-party views and AI answer cards) and labeled by the host from its **Broker URL Context**.
_Avoid_: open-in-browser tool, agent URL navigation, raw URL field
...
- A **Broker URL Context** strips the query string and fragment at the broker boundary regardless of the capture-time browser URL setting, so even a `Full`-mode stored URL yields only a guarded host+path to the broker (`http`/`https` only; `file://` collapses to `file://[local-file]`).
...
- **Open Captured URL** is app-mediated and user-triggered rather than an agent tool: it resolves the raw URL locally from the opaque result id and never crosses the broker text boundary, so the cloud model only ever sees the guarded **Broker URL Context** host.
