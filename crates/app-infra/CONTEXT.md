...
The user-facing model choice for **Semantic Search**: guided tiers (an English default and a Multilingual option) plus a Custom selection over the locally supported models. Model choice is a deliberate setup decision because changing it re-derives every **Semantic Search Vector**. A guided tier is a **curation overlay** over the fastembed catalog: the manifest pins a curated `model_code` plus presentation, token window, and license, while a model's intrinsic facts — dimension, pooling, output key, and on-disk file layout — are read from the fastembed catalog and never hand-restated, so those facts have one source. A CI guard asserts each guided tier still resolves in the catalog and that its catalog-derived facts match pinned expectations, so a fastembed drift fails loudly instead of silently shipping a stale dimension or an incomplete download list. A guided tier may pin an explicit layout override only where the catalog under-reports a model's files (proven necessary by that guard).
...
- A guided **Semantic Search Model Tier** carries only curation fields (curated `model_code`, tier, token window, license, display name, description, approximate download size); its dimension, pooling, output key, and on-disk layout are synthesized from the fastembed catalog the same way a Custom pick's are, so the guided manifest is an overlay over the catalog, not a parallel restatement of it. A guided `model_code` that no longer resolves in the catalog is a loud failure (the default model must not silently vanish), not a hand-coded fallback.
- A **Semantic Search Model**'s on-disk file list has one authority — the descriptor's `expected_layout.required_files` (the fastembed-derived union of the ONNX graph, its external-data siblings, and the four root tokenizer files) — consumed by both the install-completeness check and the download plan, so what the downloader fetches and what the completeness check requires can never diverge.
- A downloaded **Semantic Search Model** is **not checksum-verified at rest in v1**: SHA256 verification is plumbed fail-closed, but no guided-tier hashes are pinned yet (every tier currently resolves to "integrity unverified"), so guided downloads rely on TLS-to-Hugging-Face plus a Content-Length truncation guard until the per-file constants are pinned; Custom (user-entered) models are unverified by design. Pinning guided-tier hashes is a tracked follow-up.
