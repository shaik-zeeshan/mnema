    ...
    match args.as_slice() {
        ...
        [command, opaque_id] if command == "open-in-mnema" => {
            ...
            let (response, grants) = with_infra(&config_dir, |_infra, grants| async move {
                ...
                if app_infra::brokered_access::decode_opaque_id(&opaque_id).is_none() {
                    return Ok(Err(BrokerErrorResponse {
                        error:
                            app_infra::brokered_access::BrokerAuthStatusKind::AuthorizationRequired,
        ...
        _ => {
            eprintln!(
                "usage: mnema-broker auth status | search --query <text> [--limit n] | show-text <opaque-result-id> | timeline --from <ts> --to <ts> | open-in-mnema <opaque-result-id>"
            );
            process::exit(2);
        }
    }
...
fn with_infra<T, Fut>(
    config_dir: &PathBuf,
    f: impl FnOnce(app_infra::AppInfra, Vec<app_infra::brokered_access::BrokerGrant>) -> Fut,
) -> Result<(Result<T, BrokerErrorResponse>, Vec<BrokerGrant>), String>
where
    T: serde::Serialize,
    Fut: std::future::Future<Output = app_infra::Result<Result<T, BrokerErrorResponse>>>,
{
    let grants = load_grants(config_dir).map_err(|error| error.to_string())?;
    let grants = active_grants(&grants, app_infra::brokered_access::now_unix_ms());
    if grants.is_empty() {
    ...
    runtime.block_on(async {
        let infra = app_infra::AppInfra::initialize(save_directory)
            .await
            .map_err(|error| error.to_string())?;
...
fn open_mnema_deep_link(opaque_id: &str) -> app_infra::Result<()> {
