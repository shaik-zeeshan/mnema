...
- Speaker analysis has a single on-device diarization provider, **speakrs** (pure-Rust pyannote-community-1 segmentation + WeSpeaker embedding + VBx clustering on CoreML; model id `pyannote-community-1-wespeaker`). The helper runs subprocess-per-job; no persistent helper daemon, in-process model reuse, or generic audio-heavy worker abstraction is part of the current design. Because the subprocess exit is also the memory-reclamation boundary and an **Audio Segment** is capped at 5 minutes, speakrs processes the whole segment in one pass with **no safe-chunk window** — its transient CoreML peak is bounded, length-bounded scratch reclaimed on subprocess exit. (The dispatch path keeps a generic `provider` axis and the helper accepts a `--safe-chunk-ms` flag, but speakrs ignores it; it existed for a provider with its own per-call ceiling.)
