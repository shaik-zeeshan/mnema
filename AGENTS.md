...
- OCR dedupe is two-stage: `crates/capture-screen` produces a lightweight `content_fingerprint` as a prefilter, and `crates/app-infra` skips a new OCR job only when an earlier same-session frame has both the same fingerprint and exactly matching frame-file bytes.
