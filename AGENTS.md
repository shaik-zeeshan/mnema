...
- System-audio recovery is a tap **rebuild** — the one mechanism, invisible to screen/mic — triggered by device change/death, exclude-list movement, or the zero-watchdog (~30 s without sound, backoff 30 s → 600 s, resets on any sound, runs even while paused for inactivity). A rebuild finalizes the segment and opens a fresh one; never splice a tap generation into a live writer. Writer format follows the tap (device-dependent 44.1 ↔ 48 kHz, no resampler). The tap's exclude list = Mnema's own process + privacy-listed apps — **parity** with the deleted ScreenCaptureKit content filter, not a feature; a regression there silently records excluded apps. Events log under `capture_system_audio::LOG_PREFIX` (`system-audio-tap:`); drills: [`docs/agents/system-audio-drills.md`](docs/agents/system-audio-drills.md).
