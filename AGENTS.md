...
- Sensitive Capture Protection V1 stays inside the app-only privacy model from ADR 0006 and ADR 0008. Do not add URL/domain/title/private-window/password-field/browser-extension filtering to live privacy; use App Privacy Exclusion, Browser Capture Disclosure, Exclude Current App, Pause Recording, and confirmed Delete Recent Capture recovery instead.
- Sensitive Capture V2 may use first-party browser extension DOM-structure signals only as **Credential Entry Capture Suspension** through the Rust-owned **Browser Integration Runtime** from ADR 0013, not as **Live Privacy Filter** inputs, URL/domain/title/private-window filtering, page guessing, media redaction, or field-value inspection.
- Browser extension metadata signals may feed configured **Browser Metadata Collection** only; they must not drive **Live Privacy Filter** decisions or **Credential Entry Capture Suspension**.
- Browser integration safety debounce, fail-closed decisions, and recording pause/resume orchestration belong in Rust-owned native capture state and the **Recording Lifecycle**, not in Svelte.
