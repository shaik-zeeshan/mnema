...
- **Reasoning Engine** (`crates/ai-runtime`): `EngineConfig`, `extract::<T>()`, `run_agent_loop()`. Settings shape and provider-centric model selection: [ADR 0034](docs/adr/0034-ai-settings-are-provider-centric-with-one-global-default-model.md). Single resolver: `crate::ai_runtime::resolve_engine_config(settings, pin, feature_override_model)`.
- **Ask AI** (Quick Recall + Chat): in-process via `crates/ai-runtime`, implemented in `apps/desktop/src-tauri/src/ask_ai.rs`. Stateless-per-turn over the persistent conversation store. Generative-UI answer blocks (`mnema-bars`, `mnema-dossier`, `mnema-timeline`) parsed in `Chat.svelte`. See [ADR 0033](docs/adr/0033-ask-ai-migrates-onto-shared-reasoning-engine.md).
