...
- **Quick Recall** is ephemeral: each summon opens fresh on the **Quick Search** field, an in-progress **Ask AI Thread** lives only while the panel stays open, and V1 persists no conversation history to disk. A hide/re-summon inside the existing idle window resumes the live thread (its PI session stays resident for that bounded window); once the idle window elapses while hidden, the thread's PI child is killed and the next summon is fresh. Leaving **Ask AI** for **Quick Search** (Escape) abandons the thread immediately.
