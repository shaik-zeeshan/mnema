...
- An **Ask AI Thread** is **stateless-per-turn**: there is no resident model session, so each turn (including follow-ups) re-sends the thread history to the **Reasoning Engine** rather than accumulating context in a live process. Follow-up turns send no `seedQuery`; the `access.askAiMaxToolCalls` cap is per-turn (per question), not a per-thread budget. **Answer Sources**, the tool-activity summary, and the copy affordance are all per assistant turn.
