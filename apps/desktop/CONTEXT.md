...
**Ask AI Thread**:
The multi-turn conversation behind one **Ask AI** session: an ordered list of turns, each a user question plus its streamed answer, per-turn **Answer Sources**, and per-turn tool activity. The thread is **persisted** to the shared conversation store (ADR 0031), and each turn is **stateless-per-turn**: the **Reasoning Engine** loads the thread's history from the store, runs its tool-enabled agent loop, streams the answer, and persists the new turn — there is no resident model session held between turns. Only the first turn is seeded from the **Quick Search** query; follow-ups carry the raw question and re-send prior turns as context, relying on the agent's own `search`/`timeline`/`show_text` tools for anything new. A single per-in-flight-question task owns streaming and cancellation.
...
- **Ask AI** seeds the **Reasoning Engine** with **Brokered Capture Access** results for the same query the user just ran in **Quick Search**, so the user sees full-fidelity results while the (cloud or local) engine receives only redacted context.
