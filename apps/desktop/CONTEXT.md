...
**Ask AI**:
A **Quick Recall** action that pivots from a **Quick Search** query into a **Reasoning Engine** conversation that carries that query over as the *question* (plus a chip-derived scope suffix), never as pre-fetched results; the bridge from searching to asking. An Ask AI session is a multi-turn **Ask AI Thread**, not a single question/answer. Ask AI and **Insights Chat** are two doors into the same engine and conversation store: both run the in-process `rig-core` **Reasoning Engine** ([ADR 0033](../../docs/adr/0033-ask-ai-migrates-onto-shared-reasoning-engine.md)), not the retired PI shim.
_Avoid_: in-app raw-data assistant, separate chat window, agent with privileged app-infra access, a second engine distinct from the Reasoning Engine, a PI/Node runtime dependency

**Ask AI Thread**:
The multi-turn conversation behind one **Ask AI** session: an ordered list of turns, each a user question plus its streamed answer, per-turn **Answer Sources**, and per-turn tool activity. The thread is **persisted** to the shared conversation store (ADR 0031), and each turn is **stateless-per-turn**: the **Reasoning Engine** loads the thread's history from the store, runs its tool-enabled agent loop, streams the answer, and persists the new turn — there is no resident model session held between turns. No turn is pre-seeded with capture context: the first turn carries the **Quick Search** query over as its question text and follow-ups carry the raw question, and every turn relies on the agent's own `search`/`timeline`/`show_text` tools for anything new. A single per-in-flight-question task owns streaming and cancellation.
_Avoid_: a resident/warm model session between turns, a per-thread (rather than per-turn) tool-call budget, resurrect-from-transcript, a thread that exists only in memory

**Ask AI Background Completion**:
The behavior that lets an in-flight **Ask AI Thread** turn finish on its own — in **either** door, Quick Recall or **Insights Chat** — when the user navigates away, closes the surface, or blurs the panel. The answer-generating task is detached from any window: it runs to completion and **persists the finished turn to the conversation store**, so re-opening the thread reads it back. Only an **explicit cancel** stops a task; app exit aborts in-flight tasks (a partial turn at most). Returning **while a turn is still generating** is **live reattach**: the task persists incremental partial progress, and the reopened surface loads that partial then subscribes to the thread's ongoing `delta` events. See [ADR 0033](../../docs/adr/0033-ask-ai-migrates-onto-shared-reasoning-engine.md).
_Avoid_: a 30-minute unseen expiry window, releasing/killing a resident PI process, resurrect-from-transcript, "still working…" with no live tokens on return, tearing the thread down on a Quick Recall close
...
- **Ask AI** hands the **Reasoning Engine** the question only; the engine gathers its own capture context through **Brokered Capture Access** tool calls, so the user sees full-fidelity results while the (cloud or local) engine receives only redacted context.
...
- **Ask AI** is off by default and gated by a standing opt-in **Ask AI Setting** with a disclosure that questions send redacted capture context to the configured **Reasoning Engine** (the user's own cloud provider key, or a local engine); enabling the setting is the consent gate rather than a time-bounded **CLI Access Grant**, and revoking is turning the setting off. The Ask AI Setting is one of two independent feature opt-ins sitting on the shared **engine-configured** prerequisite (the other is User Context's continuous-derivation opt-in); see [ADR 0033](../../docs/adr/0033-ask-ai-migrates-onto-shared-reasoning-engine.md) and [ADR 0022](../../docs/adr/0022-ask-ai-sends-redacted-capture-context-to-cloud-agent.md).
...
- The **Ask AI** agent loop is tool-enabled: it issues every **Brokered Capture Access** query (`search`, `timeline`, `show-text`) itself, as tools during the conversation, bounded to that existing broker command set with no new tools, media paths, or raw app-infra access. `open` remains the app-mediated handoff to the dashboard rather than a data tool. The agent loop is a tool-agnostic capability of the **Reasoning Engine** crate; the broker tools, redaction, and audit are injected from the Tauri layer rather than living in the engine.
...
- **Ask AI** shares the **Reasoning Engine**'s credential: one bring-your-own-key in the OS keychain (the **Capture Index Key Store** boundary), the same key continuous derivation uses, or a reachable local engine with none. Mnema still operates no backend and no token proxy — the user's key talks straight to their provider — and conversations never transit Mnema servers. Model *selection* stays split per workload: `access.askAiModel` is a `rig-core` model id (empty → engine default) and `access.askAiMaxToolCalls` bounds per-turn data-tool calls, both independent of the background-derivation model in `AiRuntimeSettings`.
- The **Brokered Capture Access** redaction/retention boundary for **Ask AI** is enforced by the broker tools executing Rust-side (thin Tauri commands wrapping existing broker policy/query code), injected as callbacks into the engine's tool-agnostic agent loop; the engine itself never reaches capture data directly.
- **Ask AI** is unavailable when the shared **engine-configured** prerequisite is unmet (no keychain key and no reachable local endpoint, or no model selected) — the same "no usable engine → feature unavailable" shape User Context has. There is no PI/Node runtime to detect or install; a user whose Ask AI previously ran on PI's own auth must add a key in **Access Settings** before it works again. See [ADR 0033](../../docs/adr/0033-ask-ai-migrates-onto-shared-reasoning-engine.md).
- Model discovery for the **Ask AI**/Chat picker comes from the authoritative per-provider source (OpenAI/OpenAI-compatible and Anthropic `GET /v1/models`, native Ollama `GET /api/tags`), with free-form model-id entry always allowed; a failed or absent list endpoint never blocks selection.
- A Chat thread may pin a per-conversation engine identity (`{provider, model}`) chosen among **already-configured** engines, so a local-engine thread and a cloud thread can coexist; adding a new provider or key stays a Settings action, not a picker affordance. Unpinned threads and all Quick Recall use the global default engine.
...
- **Quick Recall**'s *launcher surface* is ephemeral (each summon opens fresh on the **Quick Search** field; **Quick Search** state resumes only inside the existing 5s idle window), but an **Ask AI Thread** is **persisted** to the shared conversation store (ADR 0031) and is not torn down by dismiss/blur or a full Quick Recall close: an in-flight turn finishes in the background under **Ask AI Background Completion** and re-summoning reads the saved thread back. Leaving **Ask AI** for **Quick Search** (Escape) returns to search but does not cancel a running turn; only an explicit cancel does.
- An **Ask AI Thread** is **stateless-per-turn**: there is no resident model session, so each turn (including follow-ups) re-sends the thread history to the **Reasoning Engine** rather than accumulating context in a live process. No turn is pre-seeded with capture context — every turn, first or follow-up, starts empty and the agent gathers what it needs through its own tool calls. The `access.askAiMaxToolCalls` cap is per-turn (per question), not a per-thread budget. **Answer Sources**, the tool-activity summary, and the copy affordance are all per assistant turn.
- Because turns are stateless over the persistent store, a follow-up on any thread — fresh, dismissed, or one whose last turn errored — is just another turn loaded from the store; there is no "dead thread", no resurrect-from-transcript, and no live-session-vs-replay distinction. The trade-off is that each follow-up re-sends prior turns (a modest token cost) in exchange for deleting the resident-session lifecycle. See [ADR 0033](../../docs/adr/0033-ask-ai-migrates-onto-shared-reasoning-engine.md).
...
- Pivoting to **Ask AI** inherits the visible **Quick Search** scope in natural language only: the question the agent receives gets a scope suffix derived from the active **Quick Recall Filter Chips** (`in Safari`, `from … to …`, `in microphone audio`) that also renders as the read-only question header; **Body Match Operators** stay verbatim in the residual question text. There is no structural half — nothing pre-filters the agent's evidence (see the 2026-07-26 amendment to [ADR 0025](../../docs/adr/0025-quick-recall-search-syntax-uses-launcher-native-autocomplete.md)).
...
- **Ask AI** may be invoked without a prior **Quick Search** query. A prior query is never a precondition for asking: it only carries over as the question's text plus a natural-language scope suffix derived from the active **Quick Recall Filter Chips**, never as pre-fetched broker results (see the 2026-07-26 amendment to [ADR 0025](../../docs/adr/0025-quick-recall-search-syntax-uses-launcher-native-autocomplete.md)).
