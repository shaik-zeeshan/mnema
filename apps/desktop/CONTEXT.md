...
**Ask AI**:
A **Quick Recall** action that pivots from a **Quick Search** query into a PI Agent SDK conversation seeded with the **Brokered Capture Access** (redacted) results for that same query; the bridge from searching to asking. An Ask AI session is a multi-turn **Ask AI Thread**, not a single question/answer.
_Avoid_: in-app raw-data assistant, separate chat window, agent with privileged app-infra access

**Ask AI Thread**:
The multi-turn conversation behind one **Ask AI** session: an ordered list of turns, each a user question plus its streamed answer, per-turn **Answer Sources**, and per-turn tool activity. The whole thread is backed by one live PI Agent SDK session that retains prior turns in-memory, so a follow-up is answered with the earlier turns in context. Only the first turn is seeded from the **Quick Search** query; follow-ups carry the raw question and rely on the agent's own `search`/`timeline`/`show_text` tools for anything new. Ephemeral: a thread lives only while **Quick Recall** is open and persists nothing to disk, but an **unseen** outcome survives dismiss/blur — it finishes in the background and resumes on re-summon (see **Ask AI Background Completion**).
_Avoid_: persisted chat history, a thread list, re-seeding every live follow-up, a per-thread tool-call budget

**Ask AI Background Completion**:
The lifecycle that keeps an **unseen** **Ask AI Thread** — one still streaming, or finished but never viewed while the panel was focused — alive across a dismiss/blur instead of cancelling it, so the answer finishes in the background and a re-summon lands back on it. A finished-but-unseen thread keeps its resident PI session for a fixed 30-minute window, then releases the process while leaving the transcript readable and marking the thread **expired**. A follow-up on a **dead** thread (expired, or whose last turn errored) **resurrects** it: a fresh PI session is started, re-fed the prior question/answer turns (size-capped, oldest-first) as established context, and continues into the same visible transcript. Disk-ephemeral still: nothing persists, and a full **Quick Recall** close tears the thread down. See [ADR 0027](../../docs/adr/0027-ask-ai-threads-complete-in-background-and-resurrect-from-transcript.md).
_Avoid_: persisted chat history, resume across a full Quick Recall close, replaying tool logs/Answer Sources, re-feeding the transcript while a live session exists
_Seen_: a thread is seen once a terminal turn (done/error) was rendered while the window was focused; only an unseen thread is preserved.
...
- **Quick Recall** is ephemeral: each summon opens fresh on the **Quick Search** field, an **Ask AI Thread** lives only while the panel stays open, and V1 persists no conversation history to disk. **Quick Search** state resumes inside the existing 5s idle window and is otherwise cleared; an **unseen** **Ask AI Thread**, however, is not bound to that 5s window — it survives dismiss/blur and finishes in the background under **Ask AI Background Completion** until it is seen, the 30-minute unseen cap elapses, or the panel is fully closed. Leaving **Ask AI** for **Quick Search** (Escape) abandons the thread immediately.
- An **Ask AI Thread** reuses one live PI Agent SDK session for all of its turns rather than replaying the transcript per turn, so context accumulates inside PI and only the new turn is sent to the cloud each follow-up. See [ADR 0026](../../docs/adr/0026-ask-ai-multi-turn-reuses-live-pi-session.md). Follow-up turns send no `seedQuery`; the `access.askAiMaxToolCalls` cap is per-turn (per question), not a per-thread budget. **Answer Sources**, the tool-activity summary, and the copy affordance are all per assistant turn.
- The live-session reuse holds only while the session exists; once a thread is **dead** (the 30-minute unseen helper expired, or its last turn errored) a follow-up **resurrects** it by starting a fresh session re-fed the prior Q/A transcript (size-capped at 12k chars, oldest-first), rather than calling the live-session follow-up path. This is the one place a transcript is replayed to the cloud — once, only when there is no live session to reuse — and is the narrow exception to ADR 0026's no-replay rule. See [ADR 0027](../../docs/adr/0027-ask-ai-threads-complete-in-background-and-resurrect-from-transcript.md).
