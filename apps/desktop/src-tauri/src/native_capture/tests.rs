...
use super::segments::{
    ...
    should_defer_screen_resume_for_missing_display, segment_loop_sleep_duration,
...
// A system-audio-only session is allowed: the tap needs no screen, and refusing
// one would be new coupling written into a decoupling project (ADR 0052).
#[test]
fn validate_start_request_allows_a_system_audio_only_session() {
    let request = StartNativeCaptureRequest {
    ...
    let sources = validate_start_request(&request, &support)
        .expect("system audio alone is a valid recording session");
...
#[test]
fn validate_recording_settings_allows_system_audio_without_screen() {
...
fn session_from_runtime_reports_not_running_when_screen_capture_is_broken_after_wake_failure() {
    ...
    // The screen is broken; both audio families are untouched by that and keep
    // recording (ADR 0052).
    assert_eq!(
...
fn session_from_runtime_reports_running_during_privacy_suspension_with_live_microphone() {
    ...
    let runtime = NativeCaptureRuntime {
        ...
        active_microphone_session: None,
        active_system_audio_session: None,
        capture_suspension: Some(CaptureSuspension::with_kind(
...
fn current_activity_snapshot_marks_audio_sources_enabled_from_requested_sources() {
    let _guard = audio_activity_state_test_guard();
    let mut runtime = NativeCaptureRuntime {
        is_running: true,
        requested_sources: Some(CaptureSources {
            screen: true,
...
/// Serializes the tests that read the process-global audio activity peaks.
/// `current_activity_snapshot` *drains* them, so two such tests running in
/// parallel consume each other's sample.
fn audio_activity_state_test_guard() -> std::sync::MutexGuard<'static, ()> {
...
fn current_activity_snapshot_for_debug_does_not_drain_system_audio_peak() {
    // Debug should observe the peak without draining it, and the next normal
    // take should still receive the same peak.
    let _guard = audio_activity_state_test_guard();
    capture_system_audio::reset_system_audio_activity();
    capture_system_audio::record_system_audio_activity_for_tests(0.20, 10_000, 20_000);
...
fn wake_recovery_restarts_screen_capture_and_preserves_live_microphone_output() {
        ...
        |segment_dir,
         ...
         microphone_output_path| {
            assert_eq!(
                ...
                "a wake restarts the screen alone; the tap is untouched by it"
            );
    ...
    // The tap kept writing across the wake, so its file is neither replanned nor
    // finalized here — it stays exactly as it was.
    assert_eq!(
...
// Regression: the wake path rebuilt `current_segment_output_files` from the fresh
// screen start and merged only the microphone continuation, so the file the tap
// had been writing straight through the sleep ended up tracked nowhere. The next
// rotation's `finalize_capture_outputs` then read a segment whose
// `system_audio_file` was `None`, concluded the tap's path was an untracked
// intermediate, and deleted it — up to five minutes of system audio gone on the
// first rotation after every wake, with no `audio_segment` row, no transcription
// and no error. The two sibling restart paths merged both families already.
#[cfg(target_os = "macos")]
#[test]
fn wake_recovery_carries_the_live_system_audio_file_into_the_refreshed_outputs() {
    let mut lifecycle = RecordingLifecycle::default();
    *lifecycle.runtime_mut() = running_screen_capture_runtime_fixture();
    assert!(
    ...
    assert_eq!(
        runtime.system_audio_recording_file.as_deref(),
        Some("/tmp/system-audio.m4a"),
        "the tap never stopped, so the runtime still names its file"
    );
    let outputs = runtime
        .current_segment_output_files
        .as_ref()
    ...
    assert_eq!(
        ...
        Some("/tmp/system-audio.m4a"),
        "and the segment the next rotation finalizes has to name it too, or finalize deletes it"
    );
    assert_eq!(
...
#[cfg(target_os = "macos")]
#[test]
fn wake_recovery_restarts_the_screen_alone_while_system_audio_is_paused() {
    let mut runtime = running_screen_capture_runtime_fixture();
    runtime
        .inactivity
        ...
        |_segment_dir,
         ...
         _microphone_output_path| {
            assert_eq!(
                ...
                "the screen stream carries no audio; the tap delivers its own activity and resumes itself"
            );
            Ok(resumed_segment_state_fixture(expected_screen_file.clone()))
        },
    )
...
fn system_sleep_clears_live_screen_state_but_preserves_microphone_continuation() {
    ...
    *lifecycle.runtime_mut() = running_screen_capture_runtime_fixture();
    ...
    assert!(runtime.recording_file.is_none());
    // The tap has no display dependency: a sleep leaves it recording, so its
    // in-flight file must stay tracked rather than be orphaned.
    assert_eq!(
        runtime.system_audio_recording_file.as_deref(),
        Some("/tmp/system-audio.m4a")
    );
    ...
    assert_eq!(outputs.microphone_file.as_deref(), Some("/tmp/mic.m4a"));
    assert_eq!(outputs.microphone_files, vec!["/tmp/mic.m4a".to_string()]);
    // Both audio families survive the sleep; only the screen stops.
    assert_eq!(
        runtime.current_segment_sources,
        Some(CaptureSources {
            screen: false,
...
fn wake_recovery_failure_clears_screen_bookkeeping_but_preserves_live_microphone() {
    ...
    let outputs = runtime
        .current_segment_output_files
        .as_ref()
        .expect("audio continuation should remain trackable");
    assert!(outputs.screen_file.is_none());
    // Both audio families outlive a failed screen restart, so both keep their
    // in-flight files tracked for the next boundary to commit.
    assert_eq!(
        outputs.system_audio_file.as_deref(),
        Some("/tmp/system-audio.m4a")
    );
...
fn wake_recovery_restarts_screen_capture_after_sleep_while_screen_was_paused() {
         ...
         _microphone_output_path| {
            ...
            assert_eq!(
                *sources,
                ...
                "the screen restart never plans a system-audio file; the tap owns its own"
            );
    ...
    assert!(
        outputs.system_audio_file.is_none(),
        "the screen restart contributes no system-audio file; the tap owns its own"
    );
    assert_eq!(
        runtime.current_segment_sources,
        Some(CaptureSources {
...
fn active_sources_for_inactivity_excludes_screen_when_screen_paused() {
    ...
    // Both audio families are independent of the screen session, so a screen
    // pause leaves both of them recording (ADR 0052).
...
// Regression, the ADR 0052 headline scenario: screen + system audio with the mic
// off, the user walks away, both families pause for inactivity (the tap stays
// alive by design), then the display sleeps. The suspension leaves
// `current_segment_output_files` at `None` because nothing was live. Music starts,
// system audio resumes on its own — nothing gates it on the screen any more — and
// `advance_segment` opens a real file the tap records into. The resume tracked
// that path with `as_mut()`, which silently no-ops on `None`, so the file was
// tracked nowhere: up to five minutes of real audio on disk with no
// `audio_segment` row, never transcribed, never searchable, invisible to
// retention. Both halves are pinned here — that the `None` state is reachable at
// all, and that tracking a resumed file into it creates the map rather than
// dropping the file. Driving the resume itself would need a live Core Audio tap,
// which no headless test has.
#[cfg(target_os = "macos")]
#[test]
fn a_system_audio_resume_into_a_suspended_segment_still_tracks_its_output_file() {
    let runtime_controller = running_runtime_controller();
    let runtime_state = runtime_controller.state();
    ...
    super::segments::suspend_screen_capture(
        None,
        &mut runtime,
        &CaptureErrorResponse {
            code: "capture_display_unavailable".to_string(),
            message: "display asleep".to_string(),
        },
        CaptureSuspensionKind::DisplayUnavailable,
    )
    .expect("a display sleep should suspend the screen");

    assert!(
        runtime.current_segment_output_files.is_none(),
        "with both families paused the suspension leaves no segment outputs — the state a resume lands in"
    );

    super::output::set_current_system_audio_output_file(
        super::segments::current_segment_output_files_mut(&mut runtime),
        "/tmp/system-audio-resumed.m4a".to_string(),
    );

    let outputs = runtime
        .current_segment_output_files
        .as_ref()
        .expect("tracking a resumed file must create the segment outputs, not drop it");
    assert_eq!(
        outputs.system_audio_file.as_deref(),
        Some("/tmp/system-audio-resumed.m4a")
    );
    assert_eq!(
        outputs.system_audio_files,
        vec!["/tmp/system-audio-resumed.m4a".to_string()]
    );
}
...
// The pause/resume cycle the family runs with no screen at all — the shape an
// audio-only session lives in, and the one the old resume action could never
// reach (it demanded a live ScreenCaptureKit session).
#[cfg(target_os = "macos")]
#[test]
fn system_audio_inactivity_pause_and_resume_need_no_screen() {
    let runtime_controller = running_runtime_controller();
    let runtime_state = runtime_controller.state();
    let audio_only = CaptureSources {
        screen: false,
        microphone: false,
        system_audio: true,
    };
    ...
    let mut runtime = NativeCaptureRuntime {
        is_running: true,
        requested_sources: Some(audio_only.clone()),
        current_segment_sources: Some(audio_only.clone()),
        current_segment_index: 1,
        current_segment_output_files: Some(CaptureOutputFiles {
            screen_file: None,
            screen_files: Vec::new(),
    ...
    pause_system_audio_for_inactivity(&mut runtime)
        .expect("system audio must pause with no screen session in sight");
    ...
    let outputs = runtime
        .current_segment_output_files
        .as_ref()
        .expect("segment bookkeeping survives the pause");
    assert!(outputs.system_audio_file.is_none());
    assert!(outputs.system_audio_files.is_empty());
    assert!(
        runtime.current_segment_sources.is_none(),
        "pausing the only source leaves nothing recording"
    );

    resume_system_audio_from_inactivity(&mut runtime)
        .expect("system audio must resume with no screen session in sight");

    assert!(
        !runtime.inactivity.is_system_audio_paused(),
        "the resume must not defer on screen state it no longer reads"
    );
    assert_eq!(runtime.current_segment_sources, Some(audio_only));
}

#[cfg(target_os = "macos")]
#[test]
fn resume_screen_from_inactivity_never_plans_system_audio() {
    ...
    resume_screen_from_inactivity_with_start_segment(
        &mut runtime,
        None,
        |_, screen_output, sources, _, _, _, _, _, _| {
            assert!(sources.screen);
            assert!(!sources.microphone);
            assert!(
                !sources.system_audio,
                "a screen resume starts the screen alone; the tap is already running"
            );
            assert_eq!(
                screen_output,
    ...
    .expect("screen resume should not require system-audio metadata");

    assert!(
        runtime.system_audio_planner.is_none(),
        "the system-audio planner is seeded when the tap starts, not by a screen resume"
    );
}
    ...
    resume_screen_from_inactivity_with_start_segment(
        ...
        |_segment_dir,
         ...
         _mic_path| {
            assert!(
                !sources.system_audio,
                "the screen stream carries no audio; the tap keeps delivering the activity that resumes it"
            );
            assert!(!sources.microphone);
            assert!(sources.screen);
            Ok(resumed_segment_state_fixture(expected_screen_file.clone()))
...
#[cfg(target_os = "macos")]
#[test]
fn resume_screen_leaves_the_running_system_audio_tap_alone() {
    let mut runtime = screen_paused_with_system_audio_runtime_fixture(false);
    runtime.segment_planner = Some(SegmentPlanner::with_date_prefix(
        "/tmp/native-capture-tests",
        "screen-session",
    ...
    // The tap kept recording straight through the screen pause.
    runtime.system_audio_recording_file = Some("/tmp/system-audio.m4a".to_string());
    ...
    resume_screen_from_inactivity_with_start_segment(
        &mut runtime,
        None,
        |_segment_dir, _screen_output, sources, _fr, _res, _br, _mic, _tx, _mic_path| {
            assert!(
                !sources.system_audio,
                "an unpaused system-audio family is already recording on its tap; the screen start must not re-plan it"
            );
            assert!(!sources.microphone);
            assert!(sources.screen);
    ...
    assert_eq!(
        ...
        "the screen resume must not disturb the file the tap is writing"
    );
    // ...and it must be carried into the new segment's bookkeeping, or the
    // rotation that closes it would never commit it.
    assert_eq!(
        runtime
            .current_segment_output_files
            .as_ref()
...
fn resume_screen_uses_contiguous_segment_index_when_schedule_has_advanced() {
    ...
    resume_screen_from_inactivity_with_start_segment(
        &mut runtime,
        None,
        |segment_dir, screen_output, _, _, _, _, _, _, _| {
            assert_eq!(
                segment_dir.to_string_lossy(),
                format!(
...
fn resume_audio_from_inactivity_refreshes_sources_when_screen_paused() {
    ...
    assert!(
        segment_sources.system_audio,
        "system audio resumes on its own tap while the screen stays paused"
    );
    // Screen should still be paused
    assert!(runtime.inactivity.is_screen_paused());
    assert!(!runtime.inactivity.is_any_audio_paused());

    // The resume started here with `current_segment_output_files` at `None` — the
    // state a screen pause or suspension leaves behind. Tracking the file it just
    // opened used to `as_mut()` into that `None` and silently drop it, orphaning a
    // real recording that no rotation would ever commit.
    let microphone_recording_file = runtime
        .microphone_recording_file
        .as_deref()
        .expect("a resumed microphone names the file it is writing");
    let outputs = runtime
        .current_segment_output_files
        .as_ref()
        .expect("an audio resume into an empty segment must create its output map");
    assert_eq!(
        outputs.microphone_file.as_deref(),
        Some(microphone_recording_file)
    );
}
...
fn resume_audio_from_inactivity_refreshes_sources_when_screen_paused_without_source_sessions() {
    ...
    assert!(
        segment_sources.system_audio,
        "system audio resumes on its own tap regardless of the screen"
    );
    assert_eq!(
        runtime
            .source_sessions
...
fn pause_screen_for_inactivity_preserves_sources_for_active_audio() {
    ...
    assert!(
        segment_sources.system_audio,
        "system audio keeps recording on its own tap through a screen pause"
    );
}
...
fn screen_idle_with_threshold_active_system_audio_pauses_screen_without_audio_family_pause() {
    for activity_mode in [InactivityActivityMode::SystemInputOrScreen] {
        ...
        assert_eq!(
            ...
            "an unpaused system-audio family keeps recording on its tap through a screen pause"
        );
        assert_eq!(
            runtime.system_audio_recording_file.as_deref(),
            Some("/tmp/system-audio.m4a"),
            "and the file it is still writing stays tracked, or nothing ever commits it"
        );
    }
}
...
fn current_segment_sources_for_runtime_masks_stale_screen_during_privacy_suspension() {
    ...
    assert!(
        sources.system_audio,
        "a screen suspension does not suspend the tap, so system audio stays active"
    );
}
...
// --- ADR 0052: system audio is independent of the screen ---

// Inverts the old rule. System audio used to be forced inactive whenever the
// screen was paused, because it rode the screen's stream; on its own tap it keeps
// recording through a screen pause exactly as the microphone does.
#[test]
fn active_sources_for_inactivity_keeps_system_audio_when_screen_paused() {
    let requested = CaptureSources {
        screen: true,
        microphone: false,
        system_audio: true,
    ...
    let active = active_sources_for_inactivity_paused_state(&requested, true, false, false)
        .expect("system audio alone keeps the active set non-empty");

    assert!(
        active.system_audio,
        "a paused screen must not stop system audio"
    );
    assert!(!active.screen);
}

#[test]
fn active_sources_for_inactivity_system_audio_answers_only_to_its_own_family() {
    let requested = CaptureSources {
        screen: true,
        microphone: true,
        system_audio: true,
    ...
    // Screen paused, audio families active → system audio keeps recording.
    let active = active_sources_for_inactivity_paused_state(&requested, true, false, false)
        .expect("microphone should keep sources non-empty");
    assert!(active.system_audio, "system audio needs no screen session");
    assert!(active.microphone);
    assert!(!active.screen);

    // Its own family paused → inactive, whatever the screen is doing.
    let active = active_sources_for_inactivity_paused_state(&requested, false, true, true)
...
// An audio-only session is a first-class session: no screen, no microphone.
#[test]
fn active_sources_for_inactivity_allows_a_system_audio_only_session() {
    let requested = CaptureSources {
        screen: false,
        microphone: false,
        system_audio: true,
    ...
    let active = active_sources_for_inactivity_paused_state(&requested, false, false, false)
        .expect("a system-audio-only session has an active source");
    assert_eq!(active, requested);

    assert!(
        active_sources_for_inactivity_paused_state(&requested, false, false, true).is_none(),
        "pausing the only source empties the active set"
    );
}
...
fn pause_screen_preserves_output_files_with_system_audio_and_mic() {
    // With screen+mic+system_audio, a screen pause preserves both audio families'
    // in-flight files: neither rides the screen session (ADR 0052).
    let runtime_controller = running_runtime_controller();
    let runtime_state = runtime_controller.state();
    ...
    assert_eq!(
        ...
        Some("/tmp/sys.m4a"),
        "the tap records through a screen pause, so its file stays tracked"
    );
    assert_eq!(
        output_files.microphone_file.as_deref(),
        Some("/tmp/mic.m4a")
...
// Regression: the cold screen-inactivity pause (the screen session is already
// stopped — a sleep clears it, and the tick can reach the pause before the
// did-wake handler runs) still treated system audio as screen-owned. It passed
// `system_audio_recording_file` — which the tap is still writing — into
// `finalize_capture_outputs`, which reads an open `.m4a` as having no duration,
// judges it unusable, and deletes it out from under the running writer. Every
// explicit stop path tears the tap down before committing; this path cannot,
// because ADR 0052 requires a screen pause to leave the tap alive — so the live
// file has to stay out of the commit entirely.
#[cfg(target_os = "macos")]
#[test]
fn cold_screen_pause_never_finalizes_the_file_the_tap_is_still_writing() {
    let temp = tempfile::tempdir().expect("temp dir should be created");
    ...
    // What a live AVAssetWriter's output looks like on disk: real bytes, no moov
    // atom yet, so nothing can read a duration off it.
    let system_audio_path = temp.path().join("segment-0001-system-audio.m4a");
    std::fs::write(&system_audio_path, b"open writer, no moov atom yet")
        .expect("in-flight system audio m4a should be written");
    let system_audio = system_audio_path.to_string_lossy().into_owned();
    ...
    let mut runtime = NativeCaptureRuntime {
        ...
        system_audio_recording_file: Some(system_audio.clone()),
        // Cold: no live screen session to pause, so the pause runs its
        // stop-and-commit path rather than returning early.
        active_screen_session: None,
        runtime_controller,
        runtime_state,
        inactivity: InactivityState {
    ...
    assert!(
        system_audio_path.is_file(),
        "the tap is still writing this file — a screen pause must not finalize or delete it"
    );
    assert_eq!(
        runtime.system_audio_recording_file.as_deref(),
        Some(system_audio.as_str()),
        "and it stays tracked, so the tap's next boundary still commits it"
    );
    let committed = runtime
        .output_files
        .as_ref()
    ...
    assert!(
        !committed
            ...
            .iter()
            .chain(committed.system_audio_files.iter())
            .any(|file| file == &system_audio),
        "a file still being written must not be committed as a finished segment"
    );
}
...
fn pause_screen_for_inactivity_keeps_continuation_for_system_audio_only() {
    // A system-audio-only continuation is a real continuation: the tap has no
    // screen dependency, so a screen pause leaves it recording and its file has to
    // stay tracked (ADR 0052). Before the CATap swap this asserted the opposite —
    // system audio rode the screen session, so a screen pause ended it.
    let runtime_controller = running_runtime_controller();
    let runtime_state = runtime_controller.state();
    ...
    let output_files = runtime
        .current_segment_output_files
        .as_ref()
        .expect("a live tap is an audio continuation even with no microphone");
    assert!(output_files.screen_file.is_none());
    assert_eq!(
        output_files.system_audio_file.as_deref(),
        Some("/tmp/system-audio.m4a")
...
fn resume_screen_from_inactivity_passes_dated_paths_to_start_segment_closure() {
    ...
    resume_screen_from_inactivity_with_start_segment(
        &mut runtime,
        None,
        |segment_dir, screen_output, _sources, _fr, _res, _br, _mic, _tx, _mic_path| {
            assert_eq!(
                segment_dir,
                std::path::Path::new(
...
#[cfg(target_os = "macos")]
#[test]
fn resume_screen_from_inactivity_never_carries_system_audio() {
    let expected_date_prefix = current_date_prefix();
    let runtime_controller = running_runtime_controller();
    let runtime_state = runtime_controller.state();
    ...
    resume_screen_from_inactivity_with_start_segment(
        &mut runtime,
        None,
        |segment_dir, screen_output, sources, _fr, _res, _br, _mic, _tx, _mic_path| {
            assert_eq!(
                segment_dir,
                std::path::Path::new(
                    format!("/tmp/dated-screen-resume-tests/{expected_date_prefix}/.dated-screen-session-segment-0002").as_str()
            ...
            assert!(
                !sources.system_audio,
                "the tap delivers its own activity, so the screen stream never carries audio for it"
            );
...
// ---------------------------------------------------------------------------
// Low-disk safety (ADR 0040): preflight refusal + suspend/resume transitions.
//
// `disk_space::FreeSpaceProbe` is a bare `fn` pointer, so a scripted free-space
// sequence is driven through a thread_local queue: each probe call pops the next
// reading (and repeats the last one once the queue is drained), letting a single
// test feed a decreasing-then-increasing sequence to drive the enter/stay/resume
// decisions without a real full disk.
// ---------------------------------------------------------------------------
...
mod low_disk {
    ...
    /// The next-segment estimate matching the fixtures below: 8 Mbps screen +
    /// audio over 300s. Used to derive the pause/resume thresholds for assertions.
    ...
    /// A running screen+mic+system-audio runtime whose planner roots at the system
    /// temp dir (so `measure_free_space`'s nearest-existing-ancestor walk reaches a
    /// real, statable path) and whose free-space probe is the scripted stub.
    fn low_disk_runtime_fixture() -> NativeCaptureRuntime {
    ...
    // --- Suspend / resume transitions (b) ---

    #[test]
    fn boundary_decision_enters_low_disk_below_pause() {
        let estimate = fixture_estimate();
        let pause = disk_space::pause_threshold_bytes(estimate);
        let mut runtime = low_disk_runtime_fixture();

        // Below the pause threshold -> the boundary would suspend (Pause/Critical).
        script_free_bytes(&[pause - 1]);
        let decision = super::super::segments::low_disk_decision_at_boundary(&runtime)
            .expect("a measurable reading must classify");
        assert_ne!(
            decision,
            disk_space::LowDiskDecision::Sufficient,
            "below the pause threshold the boundary must not be Sufficient"
        );

        // Strictly below the reserve floor classifies Critical (still suspends).
        ...
        // At/above the pause threshold the boundary opens the next segment.
        script_free_bytes(&[pause]);
        assert_eq!(
            super::super::segments::low_disk_decision_at_boundary(&runtime),
            Some(disk_space::LowDiskDecision::Sufficient)
        );

        // Unmeasurable -> None (best-effort: the rotation proceeds, no suspend).
        runtime.free_space_probe = Some(erroring_probe);
        assert_eq!(
            super::super::segments::low_disk_decision_at_boundary(&runtime),
            None
        );
    ...
    #[test]
    fn hysteresis_stays_suspended_between_pause_and_resume_then_resumes() {
        let estimate = fixture_estimate();
        let pause = disk_space::pause_threshold_bytes(estimate);
        let resume = disk_space::resume_threshold_bytes(estimate);
        assert!(resume > pause, "resume must sit above pause for hysteresis");
        let runtime = low_disk_runtime_fixture();

        // A reading in the hysteresis band (cleared pause, not yet resume): a
        // suspended session must NOT resume — this prevents flapping.
        let between = (pause + resume) / 2;
        script_free_bytes(&[between]);
        assert_eq!(
            super::super::segments::low_disk_can_resume(&runtime),
            Some(false),
            "must stay suspended in the hysteresis band"
        );
        ...
        // At/above the resume threshold the session resumes.
        script_free_bytes(&[resume]);
        assert_eq!(
            super::super::segments::low_disk_can_resume(&runtime),
            Some(true)
        );
    ...
    #[test]
    fn decreasing_then_increasing_sequence_drives_enter_then_resume() {
        let estimate = fixture_estimate();
        let pause = disk_space::pause_threshold_bytes(estimate);
        let resume = disk_space::resume_threshold_bytes(estimate);
        let runtime = low_disk_runtime_fixture();

        // Decreasing sequence ending below pause -> would enter LowDisk.
        script_free_bytes(&[resume + 1, pause + 1, pause - 1]);
        // Drain the first two (still sufficient) then land below pause.
        let _ = super::super::segments::low_disk_decision_at_boundary(&runtime);
        let _ = super::super::segments::low_disk_decision_at_boundary(&runtime);
        assert_ne!(
            super::super::segments::low_disk_decision_at_boundary(&runtime),
            Some(disk_space::LowDiskDecision::Sufficient),
            "the decreasing sequence must end below the pause threshold (enter LowDisk)"
        );

        // Increasing sequence reaching >= resume -> resumes.
        script_free_bytes(&[pause + 1, resume - 1, resume + 1]);
        let _ = super::super::segments::low_disk_can_resume(&runtime);
        let _ = super::super::segments::low_disk_can_resume(&runtime);
        assert_eq!(
            super::super::segments::low_disk_can_resume(&runtime),
            Some(true),
            "the increasing sequence must reach the resume threshold"
        );
    }

    #[test]
    fn low_disk_suspend_stops_microphone_but_other_kinds_keep_it() {
        let low_disk_error = CaptureErrorResponse {
            code: "capture_low_disk".to_string(),
            message: "low disk".to_string(),
        };

        // LowDisk suspends the mic too: with no live session but a live mic
        // recording file, the suspend entry must clear the mic recording file.
        let mut low_disk_runtime = NativeCaptureRuntime {
            is_running: true,
            requested_sources: Some(CaptureSources {
                screen: true,
                microphone: true,
            ...
            microphone_recording_file: Some("/tmp/low-disk-mic.m4a".to_string()),
            ..Default::default()
        };
        super::super::segments::suspend_screen_capture(
            None,
            &mut low_disk_runtime,
            &low_disk_error,
            CaptureSuspensionKind::LowDisk,
        )
        .expect("low-disk suspend should succeed with no live screen session");
        assert!(
            low_disk_runtime.microphone_recording_file.is_none(),
            "LowDisk must stop the microphone (clear its recording file)"
        );
        assert_eq!(
            low_disk_runtime
                .capture_suspension
        ...
        // PrivacyFilter keeps the mic alive: the mic continuation is preserved.
        let privacy_error = CaptureErrorResponse {
            code: "privacy_filter_apply_failed".to_string(),
            message: "privacy".to_string(),
        };
        let mut privacy_runtime = NativeCaptureRuntime {
            is_running: true,
            requested_sources: Some(CaptureSources {
                screen: true,
                microphone: true,
            ...
            microphone_recording_file: Some("/tmp/privacy-mic.m4a".to_string()),
            ..Default::default()
        };
        super::super::segments::suspend_screen_capture(
            None,
        ...
        .expect("privacy suspend should succeed with no live screen session");
        assert_eq!(
            privacy_runtime.microphone_recording_file.as_deref(),
            Some("/tmp/privacy-mic.m4a"),
            "non-LowDisk kinds must keep the microphone alive"
        );
    }

    // --- Mid-segment disk-full: partial discard + graceful stop (c) ---

    #[test]
    fn discard_partial_on_disk_full_deletes_final_paths_and_commits_no_row() {
        // Create real partial files at their final paths; the discard routine must
        // delete each one and report no committed segment row (returns false).
        let temp = tempfile::tempdir().expect("temp dir should be created");
        let screen_path = temp.path().join("segment-0001.mov");
        let mic_path = temp.path().join("segment-0001-mic.m4a");
        let sys_path = temp.path().join("segment-0001-sys.m4a");
        for path in [&screen_path, &mic_path, &sys_path] {
            std::fs::write(path, b"partial-corrupt-bytes").expect("partial file should be written");
            assert!(path.exists(), "partial fixture must exist before discard");
        }
        let screen = screen_path.to_string_lossy().into_owned();
        let mic = mic_path.to_string_lossy().into_owned();
        let sys = sys_path.to_string_lossy().into_owned();

        let output_files = CaptureOutputFiles {
            screen_file: Some(screen.clone()),
            screen_files: vec![screen.clone()],
            microphone_file: Some(mic.clone()),
            microphone_files: vec![mic.clone()],
            system_audio_file: Some(sys.clone()),
            system_audio_files: vec![sys.clone()],
        };

        let committed = super::super::segments::discard_partial_segment_on_disk_full(
            Some(&output_files),
            Some(&screen),
            Some(&mic),
            Some(&sys),
        ...
        assert!(
            !committed,
            "the disk-full discard must commit NO segment row (returns false)"
        );
        assert!(!screen_path.exists(), "screen partial must be deleted");
        assert!(!mic_path.exists(), "microphone partial must be deleted");
        assert!(!sys_path.exists(), "system-audio partial must be deleted");
    }

    #[test]
    fn discard_partial_on_disk_full_is_best_effort_for_missing_paths() {
        // A path that does not exist (NotFound) must be tolerated silently and the
        // routine must still report no committed row.
        let temp = tempfile::tempdir().expect("temp dir should be created");
        let missing = temp.path().join("never-written.mov");
        ...
        assert!(!committed, "a missing partial still commits no row");
    }

    /// A running screen+mic+system-audio runtime with a live mic recording file and
    /// a committed-outputs slot, used to drive the graceful-stop transition.
    fn stoppable_low_disk_runtime() -> NativeCaptureRuntime {
        let root = std::env::temp_dir().to_string_lossy().into_owned();
        NativeCaptureRuntime {
            is_running: true,
            requested_sources: Some(CaptureSources {
    ...
    #[test]
    fn graceful_stop_below_critical_floor_ends_session() {
        // The graceful-stop helper (with no app handle, so the notification path is
        // a no-op) must end the session: runtime no longer running, suspension
        // cleared, state Failed.
        let mut runtime = stoppable_low_disk_runtime();
        ...
        assert!(
            !runtime.is_running,
            "a below-critical graceful stop must end the session"
        );
        assert!(
    ...
    #[test]
    fn boundary_below_critical_floor_drives_graceful_stop() {
        // A scripted probe yielding free < CRITICAL_FLOOR_BYTES must drive the
        // boundary path to a graceful stop (Stopped outcome + session ended),
        // rather than a suspension. No app handle is available in a unit test, so
        // this asserts the runtime-state transition (the notification path is
        // covered by push_error_app_notification existing; it no-ops here).
        //
        // The boundary entry requires a tauri::AppHandle, so exercise the decision
        // + graceful-stop seam it composes directly: a Critical classification at
        // the boundary leads to graceful_stop_for_low_disk(commit=true).
        let mut runtime = stoppable_low_disk_runtime();
        script_free_bytes(&[disk_space::RESERVE_FLOOR_BYTES - 1]);

        let decision = super::super::segments::low_disk_decision_at_boundary(&runtime)
            .expect("a measurable below-floor reading must classify");
        assert_eq!(
            decision,
        ...
        // The Critical boundary arm performs a graceful stop. Drive that arm's
        // effect (no app handle -> notification no-op) and assert the session ends.
        super::super::segments::graceful_stop_for_low_disk(
            None,
            &mut runtime,
            disk_space::RESERVE_FLOOR_BYTES - 1,
        ...
        assert!(
            !runtime.is_running,
            "the Critical boundary path must stop the session"
        );
    }

    // --- Commit the current segment's mic audio on a low-disk suspend (b) ---

    /// Produces a real, positive-duration AAC/`.m4a` so the AVFoundation duration
    /// validation inside `finalize_capture_outputs` accepts it as usable captured
    ...
    #[test]
    fn low_disk_boundary_suspend_commits_current_segment_microphone() {
        // ADR 0040 ("the current segment is still healthy ... so commit it before
        // ending"): at a healthy rotation boundary a low-disk Pause must NOT
        // silently drop the current segment's already-finalized microphone audio.
        // The mic `.m4a` lives in current_segment_output_files (set at segment
        // start) and is a valid closed file on disk; the suspend entry must commit
        // it into runtime.output_files just as a normal rotation / inactivity
        // pause would, else up to ~5 minutes of microphone audio is orphaned (no
        // audio_segment row) on every low-disk pause.
        //
        // Mic-only session so the screen/system-audio commit path is a no-op and
        // the assertion isolates the microphone commit.
        let temp = tempfile::tempdir().expect("temp dir should be created");
        let mic_path = temp.path().join("segment-0001-mic.m4a");
        write_valid_m4a_audio_file(&mic_path);
        let mic = mic_path.to_string_lossy().into_owned();

        let mut current_segment_output_files = super::super::segments::empty_output_files();
        set_current_microphone_output_file(&mut current_segment_output_files, mic.clone());
        ...
        let mut runtime = NativeCaptureRuntime {
            ...
            current_segment_index: 1,
            microphone_recording_file: Some(mic.clone()),
            current_segment_output_files: Some(current_segment_output_files),
            output_files: Some(super::super::segments::empty_output_files()),
            ..Default::default()
        };
        ...
        let committed = runtime
            ...
            .as_ref()
            .expect("committed output_files slot should exist");
        let committed_mic = committed
            .microphone_file
            .iter()
            .chain(committed.microphone_files.iter())
            .any(|file| file == &mic);
        assert!(
            committed_mic,
    ...
    // --- Graceful stop commit gating (d) ---
    //
    // Closes the proxy-test trap the existing stop tests fall into (they assert
    // only `!is_running`, so a regression that silently dropped the committed
    // outputs would still pass). These drive a system-audio-only segment — a real,
    // AVFoundation-valid `.m4a` — so the commit runs through `finalize_capture_outputs`
    // without needing a live native session or a valid screen `.mov`.
    //
    // NB: the *screen-session-ordering* half of the graceful-stop fix (stop the
    // ScreenCaptureKit session before reading its `.mov`) cannot be unit-tested:
    // `capture-screen` exposes no constructor for a live `ActiveCaptureSession`.

    /// A running system-audio-only runtime whose current segment carries a real,
    /// committable `.m4a` at `sys_path`, used to drive the graceful-stop commit
    /// gating. No live native session is needed: the system-audio commit path only
    /// reads/validates the file.
    fn system_audio_segment_runtime(sys_path: &str, segment_index: u64) -> NativeCaptureRuntime {
    ...
    #[test]
    fn graceful_stop_commit_true_appends_current_segment_outputs() {
        // A boundary/recovery graceful stop (commit_current_segment = true) must
        // actually COMMIT the healthy current segment into runtime.output_files,
        // not merely end the session (ADR 0040 "clean-finalize what is safely
        // closable"). Asserting only `!is_running` would pass even if the commit
        // silently dropped every byte.
        let temp = tempfile::tempdir().expect("temp dir should be created");
        let sys_path = temp.path().join("segment-0002-sys.m4a");
        write_valid_m4a_audio_file(&sys_path);
        let sys = sys_path.to_string_lossy().into_owned();

        let mut runtime = system_audio_segment_runtime(&sys, 2);
        super::super::segments::graceful_stop_for_low_disk(
            None,
            &mut runtime,
            disk_space::RESERVE_FLOOR_BYTES - 1,
        ...
        assert!(
            committed_sys,
            "commit_current_segment=true graceful stop must append the healthy \
             current segment's system-audio output (got {committed:?})"
        );
        assert!(
            !runtime.is_running,
            "graceful stop must end the session"
        );
    }

    #[test]
    fn graceful_stop_commit_false_drops_current_segment_outputs() {
        // The mid-segment-fill caller passes commit_current_segment = false because
        // it already discarded its partial; graceful stop must then commit NOTHING,
        // so no half-written file is recorded. This pins the `commit` flag actually
        // gating the commit (not a vacuous parameter).
        let temp = tempfile::tempdir().expect("temp dir should be created");
        let sys_path = temp.path().join("segment-0003-sys.m4a");
        write_valid_m4a_audio_file(&sys_path);
        let sys = sys_path.to_string_lossy().into_owned();

        let mut runtime = system_audio_segment_runtime(&sys, 3);
        super::super::segments::graceful_stop_for_low_disk(
            None,
            &mut runtime,
            disk_space::RESERVE_FLOOR_BYTES - 1,
        ...
        let committed = runtime
            ...
            .expect("committed output_files slot should exist");
        assert!(
            committed.system_audio_file.is_none() && committed.system_audio_files.is_empty(),
            "commit_current_segment=false graceful stop must NOT append outputs \
             (the partial was already discarded), got {committed:?}"
        );
        assert!(!runtime.is_running, "graceful stop must end the session");
    }

    // --- Mid-segment write-failure classification (c) ---
    //
    // `classify_write_failure_disk_full` is the guard that decides whether a writer
    // append/finalize failure coincides with low disk. It is pure over the injected
    // probe, so it is testable end to end (unlike `handle_mid_segment_write_failure_*`,
    // which needs a `&tauri::AppHandle` for the notification path and is harness-blocked).

    #[test]
    fn classify_write_failure_disk_full_passes_through_on_healthy_disk() {
        // A writer failure on a HEALTHY disk must return None so the existing fatal-
        // failure behavior is preserved unchanged. If this inverted, every unrelated
        // write failure would be silently swallowed into a low-disk suspension.
        let estimate = fixture_estimate();
        let resume = disk_space::resume_threshold_bytes(estimate);
        let runtime = low_disk_runtime_fixture();
        ...
        script_free_bytes(&[resume + 1]);
        assert_eq!(
            super::super::segments::classify_write_failure_disk_full(&runtime),
            None,
            "a write failure on a healthy disk must not be treated as disk-full"
        );

        // Unmeasurable -> None too (best-effort: never force a disk-full stop on a
        // volume we cannot stat).
        let mut unmeasurable = low_disk_runtime_fixture();
        unmeasurable.free_space_probe = Some(erroring_probe);
        assert_eq!(
            super::super::segments::classify_write_failure_disk_full(&unmeasurable),
            None,
            "an unmeasurable probe must not force a disk-full classification"
        );
    }

    #[test]
    fn classify_write_failure_disk_full_detects_pause_and_critical() {
        let estimate = fixture_estimate();
        let pause = disk_space::pause_threshold_bytes(estimate);
        let runtime = low_disk_runtime_fixture();

        // Below the pause threshold but at/above the reserve floor -> Pause
        // (discard the partial and suspend).
        script_free_bytes(&[pause - 1]);
        let pause_decision = super::super::segments::classify_write_failure_disk_full(&runtime);
        assert!(
            matches!(pause_decision, Some((disk_space::LowDiskDecision::Pause, _))),
            "free below pause but above the floor must classify the failure as Pause \
             (got {pause_decision:?})"
        );

        // Below the reserve floor -> Critical (discard the partial and stop).
        script_free_bytes(&[disk_space::RESERVE_FLOOR_BYTES - 1]);
