...
// and `finalize_capture_outputs` are macOS-only because they finalize files
// emitted by ScreenCaptureKit/AVFoundation, which are not produced on other
// platforms. The `set_current_*` helpers are cross-platform pure setters.
#[cfg(target_os = "macos")]
use super::output::{
    append_committed_segment_output_files, cleanup_unusable_segment_artifacts,
...
use super::runtime::{
    ...
    screen_planner_for_runtime, NativeCaptureRuntime, SegmentLoopControl,
};
// `privacy_suspended_sources_for_runtime_state`, `PrivacyCaptureSuspension`,
// `PrivacyCaptureSuspensionStatus`, and `should_recover_from_segment_finalize_error`
// are macOS-only because they touch the privacy-filter suspension state machine
// (tied to ScreenCaptureKit) or call into macOS-only output finalization. The
// segments.rs callers of these items are themselves macOS-gated.
#[cfg(target_os = "macos")]
use super::runtime::{
    privacy_suspended_sources_for_runtime_state, should_recover_from_segment_finalize_error,
